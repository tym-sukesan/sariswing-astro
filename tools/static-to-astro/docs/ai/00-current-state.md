...
**Discography site-owner authz Slice A live Edge can_write_site probe planning (2026-08-15):** **COMPLETE (planning only)** — Phase `discography-site-owner-authz-slice-a-live-can-write-site-probe-planning` · HEAD `cdcdff7d` · VERSION 47 order: **arm → payload → JWT → can_write_site → SELECT → lock → RPC** · recommended stop `release_read_failed` on absent `discography-999` · `ARM_ON_REQUIRED: true` (Edge secret, no redeploy, UI arm off) · `DATA_WRITE_REACHABLE: false` for that packet only · `READY_FOR_OPERATOR_PROBE: false` · DB RPC PASS is **not** live Edge proof · Doc: `discography-site-owner-authz-slice-a-live-can-write-site-probe-planning.md`.
