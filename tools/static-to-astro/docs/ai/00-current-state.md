...
**G-20u45 Schedule HTTP dry-run Edge + STG client (2026-07-17):** **source complete · not deployed** — single endpoint `gosaki-schedule-save-dry-run` · `dryRun` only · save → 403 · user JWT + `is_admin` · G-9k edit lock · G-22e create `published=false` · STG click-only fetch · **REAL_EDGE_DEPLOY_EXECUTED: false** · **REAL_DRY_RUN_REQUEST_EXECUTED: false** · **SCHEDULE_SAVE_DEFAULT_DISABLED: true** · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md`. Verifier: `verify:g20u39b4-…`. **Next:** Commit/Push → Edge deploy (staging) → fresh package · FTP → operator edit/create dry-run ×1.
