# Gosaki schedule existing event venue field slice Save success finalization (G-9k6e)

**Phase:** `G-9k6e-gosaki-schedule-existing-event-venue-field-slice-save-success-finalization`  
**Status:** **complete** — operator manual G-9k6e `venue` field slice UI Save **succeeded**; documentation only (no re-Save, no DB write in this phase)  
**Date:** 2026-06-22  
**Prior:** G-9k6d `start_time` slice succeeded; G-9k6a field slice planning

| Check | Status |
| --- | --- |
| G-9k6e `venue` UI Save executed | **yes** (operator manual — once) |
...
```txt
gosakiScheduleExistingEventVenueFieldSliceSaveSuccess: true
gosakiScheduleExistingEventVenueFieldSliceSaveSuccessFinalizationComplete: true
phase: G-9k6e
readyForG9k6eVenueFieldSliceReExecution: false
...
**Do not re-click G-9k6e Save** without a new approval ID and fresh live row reconfirmation.
...
Gosaki **operator UI path** G-9k field slice Save on staging Supabase project `static-to-astro-cms-staging` **succeeded** — **`venue` only**.

| Policy | Result |
| --- | --- |
| Path | Operator UI 「変更を確認」 dry-run → 「更新する」 Save |
| Operation | **existing event UPDATE only** |
| Field slice | **`venue` only** — `changedFields: ["venue"]`, `payload keys: ["venue"]` |
| Rows | `rowsAffected: 1` |
...
| Post-save UI | **保存成功** panel visible; diff table 会場 only |

**UI note:** post-save result panel also displayed `post-save description` (informational field on record). **changedFields**, **payload keys**, and **before/after diff table** were **`venue` only** — no other field written in this slice.
...
Other safe fields (`title`, `open_time`, `start_time`, `price`, `description`) unchanged in this slice.
...
3. Edit **venue field only**
4. 「変更を確認」 — dry-run OK; `changedFields` and `payload keys` = `venue` only
5. 「更新する」 **once**
6. Post-save result panel: **保存成功**
7. Confirmed: `rowsAffected = 1`, `post-save updated_at`, `changedFields = ["venue"]`, `payload keys = ["venue"]`
8. Diff table: 会場 `川崎 ぴあにしも` → `川崎 ぴあにしも [G-9k6 venue UI保存テスト]` only
...
| Item | Value |
| --- | --- |
| **Next field** | `title` (last slice — high public visibility) |
| **Next phase** | `G-9k6f-gosaki-schedule-existing-event-title-field-slice-save-success-finalization` |
| **Baseline for G-9k6f** | `venue` = `川崎 ぴあにしも [G-9k6 venue UI保存テスト]`; `updated_at` = `2026-06-22T13:02:19.63835+00:00` |
| **Planned after** | `<Duo> [G-9k6 title UI保存テスト]` (per G-9k6a matrix) |

**Do not** combine `title` with other fields in one Save.
