...
**G-6-e1-schedule-schema-read-audit（完了）:** read-only audit plan; manual SQL for `schedules` / `schedule_months`; `auditStatus: not_run`; next: G-6-e1-schedule-schema-read-audit-result.
