...
**Status:** G-5a〜**G-5j（Sariswing admin inventory）** 完了 — G-5k 以降は段階的に  
...
**G-5j（完了）:** [admin-cms-code-inventory.md](./admin-cms-code-inventory.md) + `inventory-admin-cms.mjs` — 55 admin-related files を reusable / site-specific / risky に分類。production publish・コード変更なし。次: **G-5k** reusable admin UI components。
