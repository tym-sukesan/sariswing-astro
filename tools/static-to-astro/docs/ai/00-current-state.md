...
**CMS Core v2 YouTube Admin staging Supabase path cutover QA (2026-07-24):** **COMPLETE** — operator manual FTP full `public-dist/` → `/cms-kit-staging/gosaki-piano/` · owner login OK · `/admin/`·`/admin/youtube/` OK · YouTube display OK · Save **disabled** · no-change dry-run PASS (`ok=true` · `didWrite/dbWrite=false` · `noChange=true` · `changedItemIds=[]`) · public home YouTube **unchanged** (JSON) · other primary pages OK · production **unchanged** · Save/Secret/SQL/Edge **not** run · Gates: `cmsCoreV2YoutubeAdminStagingSupabasePathCutoverQaComplete: true` · `saveArmEnabled: false` · `publicSiteEmbedsBuildReadEnabled: false` · `ftpUploadExecuted: true` (manual only) · `readyForAnyFutureFtpApply: false` · Doc: `cms-core-v2-youtube-supabase-admin-path-package-prep.md` · **Next:** optional public build-read package (`CMS_KIT_SITE_EMBEDS_BUILD_READ`) — separate approval.
