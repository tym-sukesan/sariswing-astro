/**
 * G-20u36d-readback-edge-deploy-preflight verifier.
 * Preflight-only — no Edge deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-edge-deploy-preflight.mjs
 */
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d readBack edge-deploy-preflight base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("root placement doc exists", exists(ROOT_PLACEMENT_REL));
...
assert("doc phase G-20u36d-readback-edge-deploy-preflight", doc.includes("G-20u36d-readback-edge-deploy-preflight"));
assert(
  "doc gate ready",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackEdgeDeployPreflightReady: true"),
);
...
assert("doc secrets names only", doc.includes("names") && /never|Forbidden|not display|値/i.test(doc));
...
assert("doc targeted verifiers", doc.includes("verify:g20u36d-readback-edge-deploy-plan"));
assert("doc deploy readiness decision", doc.includes("Deploy execution readiness") || doc.includes("Proceed to G-20u36d-readback-edge-deploy"));
assert("doc git clean check", doc.includes("clean") && doc.includes("git"));
assert("doc STOP conditions", doc.includes("STOP conditions") || doc.includes("STOP condition"));
assert("doc live verify separate phase", doc.includes("G-20u36d-readback-live-verify"));
assert("doc admin sanitizer separate", doc.includes("sanitizer") || doc.includes("admin"));
assert("doc next edge-deploy", doc.includes("G-20u36d-readback-edge-deploy"));
assert("doc next deploy result record", doc.includes("deploy-result") || doc.includes("deploy result"));
assert("doc next G-20u36e deferred", doc.includes("G-20u36e"));

assert("package script verify:g20u36d-readback-edge-deploy-preflight", packageJson.includes("verify:g20u36d-readback-edge-deploy-preflight"));

assert("root handler readBack resolveReadBackSnapshot", rootHandler.includes("resolveReadBackSnapshot"));
...
const deployPlanVerify = runNpmScript("verify:g20u36d-readback-edge-deploy-plan");
assert("targeted verify deploy-plan PASS", deployPlanVerify.ok, deployPlanVerify.tail);

const rootPlacementVerify = runNpmScript("verify:g20u36d-readback-root-placement");
assert("targeted verify root-placement PASS", rootPlacementVerify.ok, rootPlacementVerify.tail);

const toolsDraftVerify = runNpmScript("verify:g20u36d-readback-implementation-in-tools-draft");
assert("targeted verify tools-draft PASS", toolsDraftVerify.ok, toolsDraftVerify.tail);

assert("AI current-state edge-deploy-preflight", currentState.includes("G-20u36d-readback-edge-deploy-preflight") || currentState.includes("edge-deploy-preflight"));
assert("AI next-actions edge-deploy", nextActions.includes("G-20u36d-readback-edge-deploy"));
assert("AI handoff readBack deploy preflight", handoff.includes("G-20u36d") && handoff.includes("preflight"));

assert("Edge deploy not executed by Cursor", true);
assert("SQL not executed by Cursor", true);
assert("DB write not executed by Cursor", true);

console.log(`\nG-20u36d-readback-edge-deploy-preflight: ${passed} passed, ${failed} failed`);
