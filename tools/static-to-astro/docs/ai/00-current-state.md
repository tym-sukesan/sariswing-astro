...
2. Current phase
現在フェーズ: **G-9g4a2-framework-single-text-field-operational-commonization-implementation**（G-9g4a2 framework planning 完了後 — implementation 未着手）

G-9g4a2 framework: Schedule single-text-field operational commonization **planning complete**（commit `e267da3` — committed and pushed）。config-driven framework for `open_time` / `start_time` / `price`; planning verifier 39 passed; manual round-trip reduction policy documented.
...
Git: branch `main`; HEAD = origin/main = `e267da3`（G-9g4a2 framework planning — committed and pushed）; working tree **clean**. G-9g4a2a restore-and-closure: commit `105c6b1`（historical）。
...
次フェーズ推奨: **G-9g4a2-framework-single-text-field-operational-commonization-implementation**

G-9g4a2 framework planning: **complete**（commit `e267da3` — committed and pushed）。Doc: `staging-shell-schedule-single-text-field-operational-commonization-planning.md`; verifier: 39 passed, 0 failed. Config-driven single-text-field operational framework for `open_time` / `start_time` / `price`. **Do not repeat** per-field manual round-trips for `start_time` / `price`. Manual non-dry-run round-trip reserved for **new common logic** only. Config-only field additions: static verifiers, guards, dry-run Preview, type checks. **Not** `start_time`-only manual execution as next slice.
...
G-9g4a2-framework-single-text-field-operational-commonization-implementation ← next (not started)
```
