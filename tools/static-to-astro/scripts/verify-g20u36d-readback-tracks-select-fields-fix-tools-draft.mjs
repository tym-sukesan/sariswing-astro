/**
 * G-20u36d-readback-tracks-select-fields-fix-tools-draft verifier.
 * Tools draft fix only — no root edit / Edge deploy / SQL / Save.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-tracks-select-fields-fix-tools-draft.mjs
 */
...
import {
  ...
  G20U36D_TRACKS_SELECT_FIELDS_FIX_PHASE,
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d tracks-select-fields fix tools-draft base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("doc phase G-20u36d-readback-tracks-select-fields-fix-tools-draft", doc.includes(G20U36D_TRACKS_SELECT_FIELDS_FIX_PHASE));
assert(
  "doc gate implemented",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackTracksSelectFieldsFixToolsDraftImplemented: true"),
);
...
assert("doc TRACK_SELECT_FIELDS without duration", doc.includes("TRACK_SELECT_FIELDS") && /remove|omit|exclude|without/i.test(doc) && doc.includes("duration"));
...
assert("handler TRACK_SELECT_FIELDS missing duration", trackSelectMissingDuration(handlerTs));
...
assert("readback lib TRACK_SELECT_FIELDS has required fields", trackSelectHasRequiredFields(readbackLib));
assert("handler mapTrackRowsToTracksText title only", handlerTs.includes("mapTrackRowsToTracksText"));
...
if (!rootPlacementComplete && rootHandler) {
  assert("root handler still includes duration (pre-placement)", trackSelectIncludesDuration(rootHandler));
...
assert(
  "package script verify:g20u36d-readback-tracks-select-fields-fix-tools-draft",
  packageJson.includes("verify:g20u36d-readback-tracks-select-fields-fix-tools-draft"),
);
...
assert(
  ...
  currentState.includes("G-20u36d-readback-tracks-select-fields-fix-tools-draft") ||
    currentState.includes("tracks-select-fields-fix-tools-draft"),
);
assert(
  "AI next-actions root placement",
  nextActions.includes("G-20u36d-readback-tracks-select-fields-fix-root-placement"),
);
assert(
  "AI handoff tracks select fields tools draft fix",
  handoff.includes("G-20u36d") && (handoff.includes("tracks-select") || handoff.includes("duration")),
);
...
console.log(`\nG-20u36d-readback-tracks-select-fields-fix-tools-draft: ${passed} passed, ${failed} failed`);
