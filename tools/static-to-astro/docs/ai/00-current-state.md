...
**Gosaki About dry-run button CSS/gate fix (2026-07-20):** **source + local browser + unarmed build** — residual STG bug after `055af17`: JS already enabled dry-run on dirty, but `.gosaki-about-admin-form__save` CSS permanently `cursor: not-allowed` / gray → dry-run looked stuck disabled. Fix: dedicated `__dry-run` / `__action` classes + `:not(:disabled)` enabled styles · `refreshSaveGate` applies dry-run UI before auth await · auth never gates dry-run button · **ABOUT_SAVE_DEFAULT_DISABLED: true** · **EXTERNAL_WRITE_EXECUTED: false** · **COMMIT_READY: true** · package regenerated · **no** FTP / Edge / HTTP Save. **Next:** Commit → operator re-upload unarmed package → STG recheck.

**Gosaki About operational edit (G-12a) (2026-07-20):** **source + mock + full build** — STG `/admin/about/` · dry-run `gosaki-about-content-dry-run` · Save `gosaki-about-content-save` · GitHub Contents JSON `gosaki-piano-about-content.json` · surgical HTML patch for form fields · client arm `PUBLIC_ADMIN_GOSAKI_ABOUT_CONTENT_WEB_SAVE_NON_DRY_RUN_ARMED` · server arm `GOSAKI_ABOUT_CONTENT_SAVE_ARMED` · approvals `G-12a-gosaki-about-content-dry-run` / `G-12a-gosaki-about-content-web-save-non-dry-run-slice` · **ABOUT_SAVE_DEFAULT_DISABLED: true** · **EXTERNAL_WRITE_EXECUTED: false** · **no** Edge deploy / live Save / FTP / DB · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md`. **Next:** Edge deploy preflight → controlled Save（operator）.
