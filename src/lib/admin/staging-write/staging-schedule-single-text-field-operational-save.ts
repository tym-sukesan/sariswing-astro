/**
 * G-9g4a2 — Gosaki site_slug single-text-field operational non-dry-run save (staging shell only).
 *
 ...
 * Safety: schedule_months untouched — update-only on public.schedules via adapter.
 */
...
import { getSingleTextFieldOperationalConfig } from "../staging-data/staging-schedule-single-text-field-operational-config";
import {
  getSingleTextFieldOperationalRegistryEntry,
  type SingleTextFieldOperationalFieldName,
} from "../staging-data/staging-schedule-single-text-field-operational-registry";
...
import {
  ...
  assertSingleTextFieldApproval,
  assertSingleTextFieldPayloadOnly,
  assertSingleTextFieldWritableRow,
...
export type SingleTextFieldOperationalPreviewBinding = {
  targetId: string;
...
export type SingleTextFieldOperationalSaveOutcome = {
  authEmail?: string;
  authStatus?: string;
  mockRole?: string;
...
function notArmedErrorCode(fieldName: SingleTextFieldOperationalFieldName): string {
  return `${fieldName}_only_not_armed`;
}

export async function executeSingleTextFieldOperationalNonDryRunSave(
  ...
  options: {
    ...
    changedFields: string[];
    previewBinding: SingleTextFieldOperationalPreviewBinding;
    candidateValues: Record<string, string>;
  },
): Promise<SingleTextFieldOperationalSaveOutcome> {
  ...
  const sliceLabel = `${entry.phasePrefix} ${entry.reclickMode}`;
  const productionSliceLabel = `${entry.phasePrefix} ${entry.label}-only`;
  ...
  if (!config.saveEnabled) {
    return {
      ...
      warnings: [],
      errorCode: notArmedErrorCode(fieldName),
  ...
  if (config.productionBlocked) {
    return {
      ...
      errorMessage: `productionBlocked: true — ${productionSliceLabel} Save refused.`,
    };
  }
  ...
  if (!options.previewBinding.hostGatePassed) {
    return {
      ...
      errorMessage: `Supabase host gate failed — ${sliceLabel} Save blocked.`,
    };
  }
  ...
  if (options.previewBinding.optimisticLockStale) {
    return {
      ...
      errorMessage: `Optimistic lock stale — re-run ${sliceLabel} Preview before Save.`,
    };
  }
  ...
  if (!options.previewBinding.previewIdentity?.trim()) {
    return {
      ...
      errorMessage: `previewIdentity required for ${sliceLabel} Save.`,
    };
  }

  if (
    options.previewBinding.consumedPreviewIdentity &&
    options.previewBinding.consumedPreviewIdentity === options.previewBinding.previewIdentity
  ) {
    return {
      optimisticLockEnabled: lockEnabled,
      expectedBeforeUpdatedAt: options.previewBinding.expectedBeforeUpdatedAt,
  ...
  try {
    assertSingleTextFieldApproval(entry, entry.approvalId, entry.phasePrefix);
    ...
    assertSingleTextFieldWritableRow(options.beforeSnapshot, entry.phasePrefix);
    assertOperationalPreviewTargetIdentity({
  ...
  if (
    ...
    return {
      ...
      errorMessage: `expectedBeforeUpdatedAt does not match latest ${sliceLabel} preview optimistic lock.`,
    };
  }
