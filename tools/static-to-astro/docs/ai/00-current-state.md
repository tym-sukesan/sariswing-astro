...
**CMS Core v2 About Supabase `save_not_armed` ok contract fix (2026-07-27):** **COMPLETE (local · undeployed)** — `...plan` after `ok:false` overwrote body `ok:true` · fixed spread order (`...plan` then `ok:false`) on root+mirror handler · 403 / `save_not_armed` / write flags false retained · other non-2xx in this Function had no same overwrite · Gates: local fix only · **EDGE_REDEPLOY_REQUIRED: true** · Doc/runbook/preflight note `ok:false` · Verifier regression in vertical-slice + deploy-preflight · **Next:** staging Edge redeploy (AGENTS) → re-run post-deploy Step 5 · no Save arm.

**CMS Core v2 About Supabase Edge post-deploy QA runbook (2026-07-26):** **READY (manual · Cursor does not invoke)** — Function `gosaki-about-supabase-save-dry-run` deployed on staging `kmjqppxjdnwwrtaeqjta` · Save arm remote **unset** · runbook: OPTIONS / 401 / dry-run 200 / didWrite false / save_not_armed 403 / allowlist 400 / seed unchanged · production **STOP** · Doc: `cms-core-v2-about-supabase-save-dry-run-edge-post-deploy-qa-runbook.md` · Gates: `ABOUT_SUPABASE_EDGE_POST_DEPLOY_QA_RUNBOOK_READY: true` · `POST_DEPLOY_QA_EXECUTED: false` · **Next:** superseded for Step5 contract by ok-fix above; redeploy then re-QA Step 5.
