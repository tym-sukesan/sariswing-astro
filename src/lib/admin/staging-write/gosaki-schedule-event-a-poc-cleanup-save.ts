/**
 * G-13d1 / G-13c1 — Gosaki Event A PoC cleanup non-dry-run save (staging shell only).
 */
...
export { G13C1_SAVE_PHASE };
import {
  executeG13c1EventAPocCleanupDryRun,
} from "./gosaki-schedule-event-a-poc-cleanup-dry-run";
import {
  assertG13c1EventAPocCleanupApproval,
  assertG13c1EventAPocCleanupBundleChangedFields,
  assertG13c1EventAPocCleanupPayloadOnly,
  assertG13c1EventAPocCleanupWritableRow,
  assertG13c1OptimisticLockBaseline,
  assertG13c1RowsAffectedExactlyOne,
...
export type G13c1EventAPocCleanupSaveBinding = {
...
export type G13c1EventAPocCleanupSaveRecordSummary = {
  id: string;
...
export type G13c1EventAPocCleanupSaveOutcome = {
  phase: typeof G13C1_SAVE_PHASE;
  approvalId: typeof G13C1_SCHEDULE_EVENT_A_POC_CLEANUP_NON_DRY_RUN_APPROVAL_ID;
  optimisticLockEnabled: boolean;
  expectedBeforeUpdatedAt: string | null;
  changedFields: string[];
  payloadKeys: string[];
  beforeRecord?: G13c1EventAPocCleanupSaveRecordSummary;
  afterRecord?: G13c1EventAPocCleanupSaveRecordSummary;
...
function toRecordSummary(row: ScheduleDryRunSource): G13c1EventAPocCleanupSaveRecordSummary {
  return {
    id: row.id,
...
export async function executeG13c1EventAPocCleanupSave(options: {
  url: string;
  anonKey: string;
  beforeSnapshot: ScheduleDryRunSource;
  formValues: G13c1EventAPocCleanupFormValues;
  saveBinding: G13c1EventAPocCleanupSaveBinding;
  env?: ImportMetaEnv;
}): Promise<G13c1EventAPocCleanupSaveOutcome> {
  const config = getG13c1EventAPocCleanupConfig(options.env ?? import.meta.env);
  const lockConfig = getScheduleOptimisticLockConfig();
  const lockEnabled = lockConfig.enabled;
  const approvalId = G13C1_SCHEDULE_EVENT_A_POC_CLEANUP_NON_DRY_RUN_APPROVAL_ID;
  const baseOutcome = {
    phase: G13C1_SAVE_PHASE,
    approvalId,
  ...
  if (!config.saveEnabled) {
    return {
      ...
      errorMessage:
        ...
        "G-13c1 Save path disabled — env arm / approval stack not satisfied.",
    };
  }
  ...
  try {
    assertG13c1EventAPocCleanupApproval(approvalId);
    ...
    assertG13c1EventAPocCleanupWritableRow(options.beforeSnapshot);
    ...
    assertG13c1OptimisticLockBaseline(options.beforeSnapshot.updated_at);

    const dryRun = executeG13c1EventAPocCleanupDryRun({
    ...
    if (!dryRun.ok) {
      throw new Error(
        `G-13c1 dry-run re-check failed: ${dryRun.guardErrors.join("; ") || "unknown"}`,
      );
    }
    if (!arraysEqual(dryRun.changedFields, options.saveBinding.changedFields)) {
      throw new Error("G-13c1 Save: changedFields mismatch vs latest dry-run.");
    }
    if (!arraysEqual(dryRun.payloadKeys, options.saveBinding.payloadKeys)) {
      throw new Error("G-13c1 Save: payload keys mismatch vs latest dry-run.");
    }

    assertG13c1EventAPocCleanupBundleChangedFields(dryRun.changedFields);
    const payload: ScheduleUpdateWritePayload = buildG13c1EventAPocCleanupPayload(
      dryRun.changedFields,
      options.formValues,
    );
    assertG13c1EventAPocCleanupPayloadOnly(payload, dryRun.changedFields);
    ...
    if (
      ...
      throw new Error("G-13c1 Save: optimistic lock baseline stale.");
    }
  } catch (err) {
  ...
  const payload = buildG13c1EventAPocCleanupPayload(
  ...
  try {
    assertG13c1RowsAffectedExactlyOne(success.rowsAffected ?? 0);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return {
      ...baseOutcome,
