...
**CMS Core v2 YouTube vertical registry FAIL resolution (2026-07-30):** **COMPLETE** — Phase `cms-core-v2-youtube-vertical-registry-fail-resolution` · 2 FAIL は live anon SELECT 依存（sandbox/network で `fetch failed` → `embedDataSource=error`）· runtime 不具合なし · verifier を registry gate 契約（`siteEmbeds=true` + `env:{}` → loader engage · live row は soft）へ修正 · Doc/AI context 最小 · **Next:** client staging share.
