...
**Status:** G-5 開始 — 実装は G-5a 以降で段階的に  
**境界:** production / Sariswing 本番非接触。今回は upload / DB update / FTP 未実施。
...
| `review-storage-assets.mjs` | `--site-slug`, `--fixture-dir`, `--data-dir`, `--report`, `--manifest` | — | — | — |
...
node tools/static-to-astro/scripts/upload-storage-assets.mjs \
  --site-config config/sites/gosaki.site-config.example.json --apply
```

`site-config-loader.mjs`（新規・G-5c）が以下を解決:
...
| 優先 | 項目 | フェーズ |
| ---: | --- | --- |
| 高 | site-config JSON schema + loader（read-only） | G-5b, G-5c |
| 高 | read-only CLI（review, plan, verify）の path 解決 | G-5c |
| 中 | schema adapter draft lib | G-5e |
| 中 | template registry + convert 連携 | G-5d, G-5f |
...
| **G-5b** | site-config draft + example JSON + schema draft | なし |
| **G-5c** | read-only CLIs site-config aware（loader、path 解決） | なし（フラグ追加のみ） |
| **G-5d** | template registry（profile → template ディレクトリ） | なし |
| **G-5e** | schema adapter lib（musician 実装 = gosaki 互換） | なし（内部 refactor） |
| **G-5f** | 新 demo site を config から generate（staging dry-run） | なし |
| **G-5g** | Admin CMS template extraction / module 分岐 | 小（template のみ） |
| **G-5h** | Product onboarding runbook + 顧客向けチェックリスト | doc のみ |

**G-5（今回）:** G-5a + G-5b の doc 固定。コード変更は example config のみ。
