...
**G-5z-c-prep（完了）:** [schema-mapping-rls-read-policy-review.md](./schema-mapping-rls-read-policy-review.md) — schema mapping + RLS review。DB query なし。次: **G-5z-c**（approval `G-5z-c-staging-read-only-data-connect`）。
