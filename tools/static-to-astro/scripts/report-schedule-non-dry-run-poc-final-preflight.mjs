#!/usr/bin/env node
/**
 * G-6-e5-schedule-non-dry-run-poc-final-preflight — Report CLI (static scan).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runScheduleNonDryRunPocFinalPreflightReport,
  writeScheduleNonDryRunPocFinalPreflightOutput,
} from "./lib/schedule-non-dry-run-poc-final-preflight-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      "Usage: node scripts/report-schedule-non-dry-run-poc-final-preflight.mjs [--out-dir PATH]",
    );
    process.exit(0);
  }
  ...
  const report = runScheduleNonDryRunPocFinalPreflightReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-schedule-non-dry-run-poc-final-preflight");
  console.log(`  phase: ${report.phase}`);
  console.log(`  finalPreflightPrepared: ${report.finalPreflightPrepared}`);
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeScheduleNonDryRunPocFinalPreflightOutput(
      absOut,
      report,
    );
  ...
  console.log("OK — schedule non-dry-run PoC final preflight report complete.");
}
