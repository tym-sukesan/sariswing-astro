...
**G-20u45-gosaki-schedule-operational-edit-ui-wiring Gosaki Schedule operational edit UI (2026-07-17):** **complete** (+ lock display fix) — STG ContentPanel list/edit/create · local dry-run preview · optimistic lock `updated_at` (**SELECT/normalize now retain `id`+`updated_at`**) · auth via CompactAuthBar + `getAccessToken` · **SCHEDULE_SAVE_DEFAULT_DISABLED: true** · package copies `gosaki-staging-schedule-operational-edit.ts` · **no fetch on display/input** · **SAVE_REQUEST_EXECUTED: false** · **DB_WRITE_EXECUTED: false** · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md` (§ G-20u45). Verifier: `verify:g20u39b4-…`. **Next:** Commit/Push → fresh STG package再生成 → manual FTP overwrite → Schedule STG recheck（lock ISO表示）.
