...
**G-20u36b-edge-dry-run-endpoint-live-verify Gosaki Discography Edge dry-run endpoint live verify (2026-07-12):** **complete** — live HTTP verify PASS on staging `gosaki-discography-save-dry-run`; Bearer anon auth · dryRun/save/siteSlug/approvalId cases verified · write flags false · service_role not exposed; **no re-deploy / SQL / Save / admin fetch POST**. Doc: `gosaki-discography-g20u36b-edge-dry-run-endpoint-live-verify.md`. Verifier: `verify-g20u36b-edge-dry-run-endpoint-live-verify.mjs` (historical). **Next:** G-20u36c admin fetch POST wiring.
