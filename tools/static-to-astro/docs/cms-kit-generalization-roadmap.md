...
**Status:** G-5a〜**G-5k（Admin UI components registry）** 完了 — G-5l 以降は段階的に  
...
| **G-5l** | Low-risk UI shell components scaffold | write-local |
| **G-5m** | Form/table CRUD + module UIs + Auth abstraction | write-local |
| **G-5n** | Media upload abstraction | staging（承認後） |
...
**G-5k（完了）:** [admin-ui-components-registry.md](./admin-ui-components-registry.md) + [`admin-ui-components-registry.json`](../config/admin/admin-ui-components-registry.json) + `inspect-admin-ui-components.mjs` — 28 UI components registry。Auth / Storage / Publish は `doNotExtractYet`。コード抽出なし。次: **G-5l** low-risk UI shell scaffold。
