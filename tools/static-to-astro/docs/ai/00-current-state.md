...
**G-20u36a-permissions-remediation-preflight-select-only Gosaki Discography permissions remediation preflight (2026-07-11):** **complete** — preflight SELECT-only SQL for grants · RLS · data baseline before manual REVOKE UPDATE x2; **READY_FOR_MANUAL_REVOKE / STOP** gate; **Cursor did not execute SQL** · **no DB write / REVOKE / GRANT / RLS change / Edge deploy / Save enable**. Doc: `gosaki-discography-g20u36a-permissions-remediation-preflight-select-only.md`. SQL: `gosaki-discography-g20u36a-permissions-remediation-preflight-select-only.sql`. Verifier: `verify-g20u36a-permissions-remediation-preflight-select-only.mjs` (historical). **Next:** preflight result record · apply plan.
