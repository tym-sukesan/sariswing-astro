...
**G-20u45 Schedule HTTP dry-run Edge + STG client (2026-07-17):** **STG edit/create endpoint dry-run verified** — `gosaki-schedule-save-dry-run` ACTIVE v1 · package `ceba4828317098b57f752c7483a949111d210a02` · edit HTTP 200 with exact optimistic lock and `changedFields=venue` · create HTTP 200 with no id/legacyId/lock, all 8 fields and `published=false` · user JWT + `is_admin` verified · both `didWrite/dbWrite/networkWrite=false` · **SCHEDULE_SAVE_REMAINS_DISABLED: true** · no Save/DB write/production change · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md`. Verifier: `verify:g20u39b4-…`. **Next:** Commit/Push → Schedule edit/create Saveを単一実装単位で実装・mock検証 → staging Edge deploy+controlled round-tripを各1回。
