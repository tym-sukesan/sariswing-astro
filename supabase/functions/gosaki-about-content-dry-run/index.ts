/**
 * G-12a — Gosaki About content dry-run (GitHub Contents GET · no writes).
 */
import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { corsHeaders, jsonResponse, requireAdminUser } from "../_shared/admin-auth.ts";
import { handleG12aAboutContentDryRunBody } from "../_shared/gosaki-about-content-dry-run.ts";
...
Deno.serve(async (req) => {
  ...
  const result = await handleG12aAboutContentDryRunBody(body);
