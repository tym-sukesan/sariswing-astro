...
**G-6-d-manual-non-dry-run-prep（完了）:** [staging-profile-manual-non-dry-run-prep.md](./staging-profile-manual-non-dry-run-prep.md) — 初回 non-dry-run 手動実行準備; bio-only; Cursor は実行しない; `readyForG6E: false`。次: ユーザー手動で1回 non-dry-run。
