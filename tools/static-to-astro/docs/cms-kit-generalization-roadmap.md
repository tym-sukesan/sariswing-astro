...
**Status:** G-5a〜**G-5h（onboarding runbook）** 完了 — G-5i 以降は段階的に  
...
| **G-5i** | Admin CMS template extraction / module 分岐 | 小（template のみ） |
| **G-5j** | customer-facing onboarding checklist | doc のみ |
| **G-5k** | pricing / subscription model draft | doc のみ |
| **G-5l** | production migration runbook | doc のみ |
| **G-5m** | first external pilot site | — |
...
**G-5h（完了）:** [cms-kit-onboarding-runbook.md](./cms-kit-onboarding-runbook.md) — 新規顧客導入の標準手順（discovery → staging → QA → production readiness → maintenance）。実装・実操作なし。次: **G-5i** Admin CMS template extraction。
