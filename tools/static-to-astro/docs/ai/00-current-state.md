...
2. Current phase
現在フェーズ: **G-9g4a1d-venue-only-operational-restore-manual-execution**（G-9g4a1c restore preflight 完了後）

G-9g4a1c: venue-only operational restore **preflight complete**（uncommitted）。marker remains; restore target `学芸大学 珈琲美学`.

G-9g4a1b1: venue-only manual execution **complete**（commit `11368be`）。target `eb1f1898-5107-4deb-a6d5-a792e0ec3f69`; venue smoke marker **remains** — restore required.
...
Git: 最新 push 済み commit `11368be`（G-9g4a1b1）。G-9g4a1c restore preflight **uncommitted**。
...
次フェーズ推奨: **G-9g4a1d-venue-only-operational-restore-manual-execution**

G-9g4a1c Schedule venue-only operational restore preflight: **complete**（uncommitted）。
...
G-9g4a1d-venue-only-operational-restore-manual-execution ← next
G-9g4a1e-venue-only-operational-restore-post-execution-hardening ← after restore
```
