...
**G-20u36e-controlled-save-auth-jwt-admin-probe-ui-stg-readonly-probe-preflight Gosaki Discography controlled Save auth JWT admin probe UI STG readonly probe preflight (2026-07-14):** **complete** — preflight only · operator JWT `rpc('is_admin')` one-click checklist · PASS/FAIL/ERROR/STOP · paste-safe fields only · **probe not clicked** · **no RPC/HTTP/SQL/GRANT/RLS/DB write/Edge/deploy/operation=save/Save/FTP re-upload** · service_role **not used**. Doc: `gosaki-discography-g20u36e-controlled-save-auth-jwt-admin-probe-ui-stg-readonly-probe-preflight.md`. Verifier: `verify:g20u36e-controlled-save-auth-jwt-admin-probe-ui-stg-readonly-probe-preflight`. Gate: **`gosakiDiscographyControlledSaveAuthJwtAdminProbeUiStgReadonlyProbePreflightPrepared: true`**. **Next:** G-20u36e-controlled-save-auth-jwt-admin-probe-ui-stg-readonly-probe-execution (after this preflight is on main).
