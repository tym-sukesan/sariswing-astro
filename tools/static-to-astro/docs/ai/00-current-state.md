...
Git: branch `main`; HEAD = origin/main = `105c6b1`（G-9g4a2a restore-and-closure — committed and pushed）; working tree **clean**. G-9g4a2a2 manual execution: commit `54623a1`（historical）。
...
G-9g4a2a Schedule open_time-only operational **restore and closure complete**（commit `105c6b1` — committed and pushed）。round-trip complete; final `open_time` `11:30`; `markerRemainsInStagingDb: false`; `activeRestoreExceptionsCount: 0`; **no further Save / restore needed**. Policy: do not repeat per-field manual round-trips for `start_time` / `price` — use common framework. Manual non-dry-run round-trip reserved for **new common logic** only. Next: config-driven single-text-field operational framework planning — **not** `start_time`-only manual execution.
...
G-9g4a2-framework-single-text-field-operational-commonization-planning ← next (not started)
```
