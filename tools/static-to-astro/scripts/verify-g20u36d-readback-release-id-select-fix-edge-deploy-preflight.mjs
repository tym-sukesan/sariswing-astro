/**
 * G-20u36d-readback-release-id-select-fix-edge-deploy-preflight verifier.
 * Preflight-only — no Edge deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-release-id-select-fix-edge-deploy-preflight.mjs
 */
...
const DOC_REL =
  "tools/static-to-astro/docs/gosaki-discography-g20u36d-readback-release-id-select-fix-edge-deploy-preflight.md";
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d release-id edge-deploy-preflight base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("old preflight doc exists (historical reference)", exists(OLD_PREFLIGHT_DOC_REL));
...
assert(
  "doc phase G-20u36d-readback-release-id-select-fix-edge-deploy-preflight",
  doc.includes("G-20u36d-readback-release-id-select-fix-edge-deploy-preflight"),
);
assert(
  "doc gate ready",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackReleaseIdSelectFixEdgeDeployPreflightReady: true"),
);
...
assert("doc old preflight FAIL reason", doc.includes("64/65") || doc.includes("historical"));
assert(
  "doc old preflight historical root-placement mismatch",
  doc.includes("verify:g20u36d-readback-root-placement") ||
    doc.includes("historical root-placement verifier"),
);
assert(
  "doc release-id fix verifiers PASS premise",
  doc.includes("verify:g20u36d-readback-release-id-select-fix-root-placement") &&
    doc.includes("verify:g20u36d-readback-release-id-select-fix-tools-draft"),
);
...
assert("doc GOSAKI_DISCOGRAPHY_DRY_RUN_READBACK_ENABLED added", doc.includes("GOSAKI_DISCOGRAPHY_DRY_RUN_READBACK_ENABLED") && /added|exists/i.test(doc));
...
assert("doc targeted verifier root-placement", doc.includes("verify:g20u36d-readback-release-id-select-fix-root-placement"));
assert("doc targeted verifier tools-draft", doc.includes("verify:g20u36d-readback-release-id-select-fix-tools-draft"));
assert("doc targeted verifier plan", doc.includes("verify:g20u36d-readback-release-id-select-fix-plan"));
assert("doc targeted verifier env-secret-result", doc.includes("verify:g20u36d-readback-env-secret-setting-result"));
assert("doc do not use old preflight as gate", doc.includes("verify:g20u36d-readback-edge-deploy-preflight") && /Do not use|historical/i.test(doc));
assert("doc STOP conditions", doc.includes("STOP conditions") || doc.includes("STOP condition"));
assert("doc deploy readiness decision", doc.includes("Deploy execution readiness") || doc.includes("Proceed to G-20u36d-readback-release-id-select-fix-edge-deploy"));
...
assert(
  "package script verify:g20u36d-readback-release-id-select-fix-edge-deploy-preflight",
  packageJson.includes("verify:g20u36d-readback-release-id-select-fix-edge-deploy-preflight"),
);
...
const rootPlacementVerify = runNpmScript("verify:g20u36d-readback-release-id-select-fix-root-placement");
assert("targeted verify release-id root-placement PASS", rootPlacementVerify.ok, rootPlacementVerify.tail);

const toolsDraftVerify = runNpmScript("verify:g20u36d-readback-release-id-select-fix-tools-draft");
assert("targeted verify release-id tools-draft PASS", toolsDraftVerify.ok, toolsDraftVerify.tail);

const planVerify = runNpmScript("verify:g20u36d-readback-release-id-select-fix-plan");
assert("targeted verify release-id plan PASS", planVerify.ok, planVerify.tail);
...
assert(
  "AI current-state release-id edge-deploy-preflight",
  currentState.includes("G-20u36d-readback-release-id-select-fix-edge-deploy-preflight") ||
    currentState.includes("release-id-select-fix-edge-deploy-preflight"),
);
assert(
...
assert(
  "AI handoff release id deploy preflight",
  handoff.includes("release-id") && (handoff.includes("preflight") || handoff.includes("edge-deploy")),
);
...
console.log(
  `\nG-20u36d-readback-release-id-select-fix-edge-deploy-preflight: ${passed} passed, ${failed} failed`,
);
