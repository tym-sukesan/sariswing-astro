...
**G-5x（完了）:** [staging-runtime-shell-integration.md](./staging-runtime-shell-integration.md) — `/__admin-staging-shell/musician-basic/`。`ENABLE_ADMIN_STAGING_SHELL=true` + dev only。shell-only / actions disabled。次: **G-5y** Supabase Auth staging integration。
