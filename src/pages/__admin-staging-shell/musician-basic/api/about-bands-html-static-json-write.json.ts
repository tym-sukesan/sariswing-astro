/**
 * POST /__admin-staging-shell/musician-basic/api/about-bands-html-static-json-write.json
 * G-10h4c — Gosaki About bands HTML static JSON write dry-run (staging shell only).
 */
...
} from "../../../../lib/admin/staging-write/gosaki-about-bands-html-static-json-write-types";
...
async function handlePost(request: Request): Promise<Response> {
  ...
  const writeConfig = getG10h4cAboutBandsHtmlStaticJsonWriteConfig(env);
    ...
    return jsonResponse(
      {
        ...
        errorMessage: "About bands HTML write API is disabled outside staging shell dev.",
      },
      403,
    );
  }
  ...
  const result = executeG10h4cAboutBandsHtmlStaticJsonWriteDryRun({
    cwd: process.cwd(),
    approvalId,
    siteSlug,
    blockId,
