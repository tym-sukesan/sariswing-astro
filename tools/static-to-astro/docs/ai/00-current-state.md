...
**Gosaki Wix external assets localization (2026-08-18):** **COMPLETE (local source + verify · no package / FTP / commit)** — Phase `gosaki-wix-external-assets-localization` · 13 `wixstatic` media IDs + parastorage favicon localized (14 files, JPEG/PNG/ICO as-is) · public HTML rewrite via gosaki `applyPostGenerate` (`BASE_URL` + `/images/wix-local/`) · `gosakirikakotrio.wixsite.com` **kept** · fixture Thunderbolt leftovers not public · package verifier fail-closed `public Wix CDN media dependency = 0` · ciao-preview remote **stale** until later regen · `READY_FOR_COMMIT_PUSH: true` · `READY_FOR_PREVIEW_REGENERATION: false` · Next Primary: `gosaki-wix-assets-localization-commit-push` · Doc: `gosaki-wix-external-assets-localization.md`.
