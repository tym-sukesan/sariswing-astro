/**
 * G-10g2 — Gosaki Contact HubSpot layout fix package prep verification.
 * Run: node tools/static-to-astro/scripts/verify-g10g2-gosaki-contact-hubspot-layout-fix-package-prep.mjs
 */
...
const DOC_REL = "tools/static-to-astro/docs/gosaki-contact-hubspot-layout-fix-package-prep.md";
const OVERRIDES_REL =
...
assert("G-10g2 doc phase", doc.includes("G-10g2-gosaki-contact-hubspot-layout-fix-package-prep"));
assert("prep complete gate", doc.includes("gosakiContactHubspotLayoutFixPackagePrepComplete: true"));
...
assert("G-10g2 layout CSS block", overridesSrc.includes("G-10g2 gosaki contact HubSpot PC layout fix"));
assert("hubspot embed wrapper class", overridesSrc.includes(".gosaki-contact-hubspot-embed"));
assert("PC grid columns", overridesSrc.includes("grid-template-columns: minmax(209px, 360px)"));
assert("PC min-width media query", overridesSrc.includes("@media (min-width: 769px)"));
assert("contact mesh container scoped", overridesSrc.includes("comp-lol1i5gqinlineContent-gridContainer"));
assert("form max-width 640px", overridesSrc.includes("max-width: 640px"));
...
if (contactHtml) {
  ...
  assert(
    "public-dist G-10g2 layout CSS in bundle",
    publicCssBundle.includes("gosaki-contact-hubspot") &&
      publicCssBundle.includes("minmax(209px") &&
      publicCssBundle.includes("comp-lol1i5gqinlineContent"),
  );
}

assert("global.css G-10g2 block", globalCss.includes("G-10g2 gosaki contact HubSpot PC layout fix"));
...
assert("00-current-state G-10g2", read("tools/static-to-astro/docs/ai/00-current-state.md").includes("G-10g2"));
...
console.log(`\nG-10g2 verification: ${passed} passed, ${failed} failed`);
