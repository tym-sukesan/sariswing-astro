...
**G-20u26 Gosaki staging package regen after discography filtered read (2026-07-10):** **complete** — `build:gosaki:staging` + `preflight:gosaki:staging` **PASS** at `72b064c`; MANIFEST `sourceCommit=72b064c` · `includesAdmin: true` · August **14** cards · discography **4** releases / **34** tracks (filtered `site_slug=gosaki-piano`); sitemap no `/admin/`; **no FTP / deploy / package upload**. Doc: `gosaki-staging-package-regen-after-discography-filtered-read.md`. Verifier: `verify-g20u26-gosaki-staging-package-regen-after-discography-filtered-read.mjs` (historical HEAD-pinned). Upload: **manual FTP only**; production STOP (G-20j).
