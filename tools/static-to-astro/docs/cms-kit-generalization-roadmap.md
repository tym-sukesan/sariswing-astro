...
**G-5z-a（完了）:** [read-only-data-integration-plan.md](./read-only-data-integration-plan.md) — read-only data integration plan。DB query なし。RLS review 必須。production data 禁止。次: **G-5z-b** mock adapter scaffold。
