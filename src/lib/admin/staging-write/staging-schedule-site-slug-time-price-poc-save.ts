/**
 * G-9g3c — Gosaki site_slug open_time + start_time + price non-dry-run PoC save (staging shell only).
 */
...
import { getG9G3cTimePricePocConfig } from "../staging-data/staging-schedule-site-slug-time-price-poc-config";
...
import {
  ...
  assertG9G3cTimePricePayloadOnly,
...
export type G9G3cTimePriceBeforeSnapshotCheckResult = {
  ok: boolean;
  abortReason?: string;
  warnings: string[];
...
export function validateG9G3cTimePriceBeforeSnapshot(
  row: ScheduleDryRunSource,
): G9G3cTimePriceBeforeSnapshotCheckResult {
...
export type G9G3cTimePriceNonDryRunSaveOutcome = {
  preCheck: G9G3cTimePriceBeforeSnapshotCheckResult;
  authEmail?: string;
  authStatus?: string;
  mockRole?: string;
...
export async function executeG9G3cTimePriceNonDryRunSave(options: {
  url: string;
  anonKey: string;
  beforeSnapshot: ScheduleDryRunSource;
  payload: ScheduleUpdateWritePayload;
}): Promise<G9G3cTimePriceNonDryRunSaveOutcome> {
  ...
  if (!config.saveEnabled) {
    return {
      preCheck: {
        ok: false,
        abortReason:
          config.armFailureReason ?? "G-9g3c time+price PoC gates not satisfied.",
        warnings: [],
      },
      ...
      errorMessage: config.armFailureReason ?? "G-9g3c time+price PoC not armed.",
    };
  }

  const preCheck = validateG9G3cTimePriceBeforeSnapshot(options.beforeSnapshot);
  if (!preCheck.ok) {
    return {
      preCheck,
      optimisticLockEnabled: lockEnabled,
  ...
  try {
    assertG9G3cTimePricePayloadOnly(options.payload);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return {
      preCheck,
