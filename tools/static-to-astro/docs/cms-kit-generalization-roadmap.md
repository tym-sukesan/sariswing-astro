...
**Status:** G-5a〜**G-5l（Admin UI shell scaffold）** 完了 — G-5m 以降は段階的に  
...
**G-5l（完了）:** [admin-ui-shell-scaffold.md](./admin-ui-shell-scaffold.md) — 10 low-risk components（AdminLayout 等）in `templates/admin-cms/`。Sariswing 未変更、`productionReady: false`。次: **G-5m** CRUD primitives + module UIs。
