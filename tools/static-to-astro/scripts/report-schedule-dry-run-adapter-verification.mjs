#!/usr/bin/env node
/**
 * G-6-e3-schedule-dry-run-adapter-verification — Report CLI (verification scan only; no DB access).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runScheduleDryRunAdapterVerificationReport,
  writeScheduleDryRunAdapterVerificationOutput,
} from "./lib/schedule-dry-run-adapter-verification-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      `Usage: node scripts/report-schedule-dry-run-adapter-verification.mjs [--out-dir PATH]`,
    );
    process.exit(0);
  }
  ...
  const report = runScheduleDryRunAdapterVerificationReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-schedule-dry-run-adapter-verification");
  ...
  console.log(
    `  readyForG6E3ScheduleDryRunAdapterManualVerification: ${report.readyForG6E3ScheduleDryRunAdapterManualVerification}`,
  );
  console.log(
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeScheduleDryRunAdapterVerificationOutput(
      absOut,
      report,
    );
  ...
  console.log("OK — schedule dry-run adapter verification report complete.");
}
