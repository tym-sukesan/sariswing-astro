# Gosaki client preview feedback closure (G-9h1)

**Phase:** `G-9h1-gosaki-client-preview-feedback-closure`  
**Status:** checklist prepared — **client feedback not yet collected**  
**Date:** 2026-06-19  
**Prior:** G-9h planning commit `a0e1b68`  
...
| 5 | **YouTube 埋め込み** — 現状どおりでよいか / CMS 管理したいか | `[ ]` | 現状は Wix 由来の静的表示。CMS 化は別フェーズ（G-9i） |
...
| 8 | 件数の体感（目安: 3月13 / 4月10 / 5月12 / 6月11 / 7月14） | `[ ]` | 全月をざっと確認 |
...
| 27 | 埋め込み URL（チャンネル or 特定動画） | `[ ]` | 未提供なら G-9i で計画 |
| 28 | 本人が URL だけ差し替えできれば十分か | `[ ]` | G-9a MVP #2 の想定 |
...
| Schedule データ | staging DB 60 行 `site_slug=gosaki-piano` + convert fallback 整合 | G-9b–G-9d |
...
- ライブ HTML は `scheduleDataSource=static-fallback` ビルドの可能性あり（内容は DB seed と整合 — G-9h2 で marker 確認予定）
...
| 6 | **YouTube** — 埋め込み要否と URL | G-9a で計画のみ |
...
| 順 | フェーズ | 内容 | DB write |
| --- | --- | --- | --- |
| 1 | **G-9h2** | Public schedule read 検証 + manual re-upload planning | No |
| 2 | **G-9h3** | Schedule CMS practicalization Phase 1/2 境界の確定 | No |
| 3 | **表示修正（A 分類）** | G-8 系 overrides + manual re-upload | No |
| 4 | **G-9i** | Top YouTube embed planning | No（計画のみ） |
| 5 | **Phase 2** | Schedule CMS write slices（明示承認後） | 別ゲート |
...
readyForG9h2PublicScheduleReadVerification: false
...
| 成果物 | 内容 |
| --- | --- |
| `gosaki-client-preview-feedback-closure-result.md`（予定） | 分類済み残差リスト、ゲート更新、G-9h2 推奨 |
