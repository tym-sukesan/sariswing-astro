/**
 * G-20u36d-readback-tracks-select-fields-fix-root-placement verifier.
 * Root placement only — no Edge deploy / SQL / Save.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-tracks-select-fields-fix-root-placement.mjs
 */
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d tracks-select-fields root-placement base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert(
  "doc phase G-20u36d-readback-tracks-select-fields-fix-root-placement",
  doc.includes("G-20u36d-readback-tracks-select-fields-fix-root-placement"),
);
assert(
  "doc gate root placed",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackTracksSelectFieldsFixRootPlaced: true"),
);
...
assert("tools draft TRACK_SELECT_FIELDS missing duration", trackSelectMissingDuration(draftHandler));
...
assert(
  "package script verify:g20u36d-readback-tracks-select-fields-fix-root-placement",
  packageJson.includes("verify:g20u36d-readback-tracks-select-fields-fix-root-placement"),
);

assert(
  "AI current-state tracks-select-fields root placement",
  currentState.includes("G-20u36d-readback-tracks-select-fields-fix-root-placement") ||
    currentState.includes("tracks-select-fields-fix-root-placement"),
);
assert(
  "AI next-actions edge deploy preflight",
  nextActions.includes("G-20u36d-readback-tracks-select-fields-fix-edge-deploy-preflight") ||
    nextActions.includes("tracks-select-fields-fix-edge-deploy-preflight"),
);
assert(
  "AI handoff tracks select fields root placement",
  handoff.includes("tracks-select") || handoff.includes("duration"),
);
...
console.log(
  `\nG-20u36d-readback-tracks-select-fields-fix-root-placement: ${passed} passed, ${failed} failed`,
);
