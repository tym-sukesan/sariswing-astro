/**
 * G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight verifier.
 * Preflight-only — no Edge deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight.mjs
 */
...
const LIVE_VERIFY_RETRY2_DOC_REL =
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d tracks-relation filter-fix edge-deploy-preflight base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("live verify retry-2 doc exists", exists(LIVE_VERIFY_RETRY2_DOC_REL));
...
assert(
  "doc phase G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight",
  doc.includes("G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight"),
);
assert(
  "doc gate ready",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackTracksRelationFilterFixEdgeDeployPreflightReady: true"),
);
...
assert("doc production ref STOP", doc.includes(PRODUCTION_REF) && /STOP|forbidden|禁止|unused/i.test(doc));
...
assert(
  ...
  doc.includes("verify:g20u36d-readback-tracks-relation-filter-fix-root-placement"),
);
assert(
  "doc targeted verifier tools-draft",
  doc.includes("verify:g20u36d-readback-tracks-relation-filter-fix-tools-draft"),
);
assert("doc targeted verifier env-secret-result", doc.includes("verify:g20u36d-readback-env-secret-setting-result"));
assert("doc do not use historical select-fields preflight as gate", doc.includes("tracks-select-fields-fix-edge-deploy-preflight") && /Do not use|not this deploy gate|historical/i.test(doc));
...
assert(
  ...
  doc.includes("Deploy execution readiness") ||
    doc.includes("Proceed to G-20u36d-readback-tracks-relation-filter-fix-edge-deploy"),
);
assert("doc next edge-deploy", doc.includes("G-20u36d-readback-tracks-relation-filter-fix-edge-deploy"));
assert(
  "doc next live verify retry-3",
  doc.includes("G-20u36d-readback-live-verify-retry-3") || doc.includes("live verify retry-3"),
);
assert("doc G20u36e after retry-3 PASS", doc.includes("G-20u36e") && /retry-3|after/i.test(doc));
assert("doc post-deploy trackCount 8", doc.includes("trackCount") && doc.includes("8"));
assert(
  "doc live verify retry-3 not executed",
  doc.includes("retry-3") && /not executed|NOT this phase/i.test(doc),
);

assert(
  "package script verify:g20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight",
  packageJson.includes("verify:g20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight"),
);
...
const rootPlacementVerify = runNpmScript("verify:g20u36d-readback-tracks-relation-filter-fix-root-placement");
assert("targeted verify relation root-placement PASS", rootPlacementVerify.ok, rootPlacementVerify.tail);

const toolsDraftVerify = runNpmScript("verify:g20u36d-readback-tracks-relation-filter-fix-tools-draft");
assert("targeted verify relation tools-draft PASS", toolsDraftVerify.ok, toolsDraftVerify.tail);
...
assert(
  "AI current-state relation edge-deploy-preflight",
  currentState.includes("G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight") ||
    currentState.includes("tracks-relation-filter-fix-edge-deploy-preflight"),
);
assert(
  "AI next-actions edge-deploy",
  nextActions.includes("G-20u36d-readback-tracks-relation-filter-fix-edge-deploy") ||
    nextActions.includes("tracks-relation-filter-fix-edge-deploy"),
);
assert(
  "AI handoff relation deploy preflight",
  handoff.includes("tracks-relation-filter-fix-edge-deploy-preflight") ||
    (handoff.includes("discography_legacy_id") && handoff.includes("preflight")),
);

assert("Edge deploy not executed by Cursor", true);
assert("SQL not executed by Cursor", true);
assert("DB write not executed by Cursor", true);
assert("live verify retry-3 not executed", true);
assert("G20u36e controlled Save planning not ready", true);

console.log(
  `\nG-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight: ${passed} passed, ${failed} failed`,
);
