# Staging shell schedule venue-only operational expansion manual execution result (G-9g4a1b1)

**Phase:** `G-9g4a1b1-venue-only-operational-expansion-manual-execution`  
**Status:** **complete**  
**Date:** 2026-06-19  
**Prior:** G-9g4a1 Save gate sync fix — commit `78888f5`; G-9g4a1b execution runbook — commit `6564061`  
**Type:** operator manual non-dry-run Save — **one UPDATE on staging `public.schedules` (venue only)**
...
| DB write executed | **yes** (one row, venue only) |
...
Cursor did **not** click Save or Preview. Operator performed Preview + Save manually.

**Do not re-click G-9g4a1 venue-only Save on this row without fresh Preview.** **Do not re-run G-9g2 / G-9g3b / G-9g3c / G-9g3d PoC Save.**
...
**Recommended next:** `G-9g4a1c-venue-only-operational-restore-preflight`
...
G-9g4a1 ARMED: true
Preview dry-run: PASS (operator manual, G-9g4a1 venue-only path)
...
venue changed: yes (G-9g4a1 venue smoke marker appended)
...
restore required: yes (venue smoke marker must be removed)
```
...
```txt
before: 2026-06-16T16:03:41.551792+00:00
after:  2026-06-19T05:12:41.853845+00:00
```
...
```txt
markerRemainsInStagingDb: true
current venue in staging DB: 学芸大学 珈琲美学 [G-9g4a1 venue smoke]
restore target venue:        学芸大学 珈琲美学
restore required: yes
restore executed: no
```

Published row — venue smoke marker may appear on public schedule until restore completes.

**Do not execute restore in this phase.** Next: `G-9g4a1c-venue-only-operational-restore-preflight`.
...
**`G-9g4a1c-venue-only-operational-restore-preflight`**
...
Lock baseline for restore preflight: `updated_at` = `2026-06-19T05:12:41.853845+00:00`
