-- =============================================================================
-- Gosaki Discography operational Save RPC — Slice A can_write_site gate (FORWARD)
...
--
-- Replaces legacy public.is_admin() gate with:
--   exact singleton sites resolve by site_slug → public.can_write_site(site_id)
-- SECURITY DEFINER retained for album+tracks atomic TX (RLS bypass) — authz mandatory inside.
...
BEGIN
  -- 1) Input identity (reject empty; bind all writes to trimmed site + legacy)
  ...
  -- 2) Site-scoped authz (REQUIRED inside SECURITY DEFINER — do not use is_admin)
  SELECT count(*)::integer
  INTO v_site_count
  FROM public.sites s
  WHERE s.site_slug = v_site_slug;

  IF v_site_count IS DISTINCT FROM 1 THEN
    RETURN jsonb_build_object(
      'ok', false,
      'reason_code', CASE
        WHEN coalesce(v_site_count, 0) = 0 THEN 'site_resolve_failed'
        ELSE 'site_resolve_ambiguous'
      END,
      'message', 'sites.site_slug must resolve to exactly one row',
      'http_status', CASE WHEN coalesce(v_site_count, 0) = 0 THEN 503 ELSE 409 END
    );
  END IF;

  SELECT s.id, s.status
  INTO v_site_id, v_site_status
  FROM public.sites s
  WHERE s.site_slug = v_site_slug;
  ...
  IF v_site_id IS NULL THEN
    RETURN jsonb_build_object(
      'ok', false,
      'reason_code', 'site_resolve_failed',
      'message', 'sites.id missing after resolve',
      'http_status', 503
    );
  END IF;

  IF v_site_status IS DISTINCT FROM 'active' THEN
    RETURN jsonb_build_object(
      'ok', false,
      'reason_code', 'site_suspended',
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RETURN jsonb_build_object(
        'ok', false,
        'reason_code', 'can_write_site_probe_failed',
  ...
  IF v_can_write IS DISTINCT FROM true THEN
    RETURN jsonb_build_object(
      'ok', false,
      'reason_code', 'can_write_site_denied',
...
COMMENT ON FUNCTION public.gosaki_discography_operational_save(
  ...
  'Gosaki staging atomic Discography operational Save. SECURITY DEFINER; requires can_write_site(sites.id) after exact site_slug resolve; no table grants to authenticated.';
...
-- Slice A RLS writer SELECT is a separate template.
