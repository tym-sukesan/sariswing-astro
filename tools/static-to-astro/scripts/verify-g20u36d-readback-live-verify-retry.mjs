/**
 * G-20u36d-readback-live-verify-retry
 * Live HTTP verify retry — readBack anon SELECT on staging Edge Function (read-only · no DB write · no secrets logged).
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-live-verify-retry.mjs
 */
...
const DOC_REL = "tools/static-to-astro/docs/gosaki-discography-g20u36d-readback-live-verify-retry.md";
const DEPLOY_RESULT_DOC_REL =
  "tools/static-to-astro/docs/gosaki-discography-g20u36d-readback-release-id-select-fix-edge-deploy-result.md";
const PRIOR_LIVE_VERIFY_DOC_REL =
...
/** @type {{ matching?: object, plusOne?: object, saveReject?: object, wrongSlug?: object, dbTrackCount?: number, retryPassed?: boolean }} */
export const liveVerifyRetryCapture = {};
...
async function runLiveVerifyRetryCases(anonKey, supabaseUrl) {
  const authHeaders = {
  ...
  liveVerifyRetryCapture.dbTrackCount = dbSummary.trackCount;
  ...
  liveVerifyRetryCapture.matching = matchingSummary;
  ...
  const retryPassed = trackCount === EXPECTED_TRACK_COUNT;
  ...
  liveVerifyRetryCapture.plusOne = plusOneSummary;
  ...
  liveVerifyRetryCapture.saveReject = saveSummary;
  ...
  liveVerifyRetryCapture.wrongSlug = wrongSlugSummary;
  ...
  liveVerifyRetryCapture.retryPassed = retryPassed && failed === 0;
  return retryPassed;
}

function printCaseSummaryTable() {
  console.log("\n--- Live verify retry case summary (sanitized) ---");
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d readBack live-verify-retry base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("live verify retry doc exists", exists(DOC_REL));
...
const retryPassedLive = await runLiveVerifyRetryCases(anonKey, supabaseUrl);
printCaseSummaryTable();

const doc = read(DOC_REL);
const docGatePass = doc.includes("gosakiDiscographyEdgeDryRunReadBackLiveVerifyRetryPassed: true");
const docGateStop = doc.includes("gosakiDiscographyEdgeDryRunReadBackLiveVerifyRetryPassed: false");

assert("doc phase G-20u36d-readback-live-verify-retry", doc.includes("G-20u36d-readback-live-verify-retry"));
assert(
  "doc gate matches live outcome",
  retryPassedLive ? docGatePass : docGateStop,
  `live retryPassed=${retryPassedLive} docPass=${docGatePass} docStop=${docGateStop}`,
);
...
if (retryPassedLive) {
  ...
  assert("doc matching status 400 or STOP", (doc.includes("400") && doc.includes("matching")) || doc.includes("STOP"));
...
assert("doc plusOne status 200", doc.includes("200") && (doc.includes("+1") || doc.includes("plusOne") || doc.includes("plus-one")));
...
assert(
  "doc next phase",
  doc.includes("G-20u36e") || doc.includes("tracks-select") || doc.includes("admin-sanitizer") || doc.includes("STOP"),
);
...
assert("npm verify script", packageJson.includes("verify:g20u36d-readback-live-verify-retry"));

const currentState = read(`${AI_DIR}/00-current-state.md`);
const nextActions = read(`${AI_DIR}/03-next-actions.md`);
const handoff = read(`${AI_DIR}/handoff-to-chatgpt.md`);
assert(
  "AI current-state live-verify-retry",
  currentState.includes("G-20u36d-readback-live-verify-retry") || currentState.includes("live-verify-retry"),
);
assert(
  "AI next-actions after live-verify-retry",
  nextActions.includes("live-verify-retry") ||
    nextActions.includes("tracks-select") ||
    nextActions.includes("G-20u36e"),
);
assert(
  "handoff live-verify-retry",
  handoff.includes("live-verify-retry") || handoff.includes("LiveVerifyRetry"),
);
...
if (retryPassedLive) {
  console.log("\nRESULT: live verify retry PASS — gate may be true");
} else {
  console.log("\nRESULT: live verify retry PARTIAL STOP — gate false documented");
}

console.log(`\nG-20u36d-readback-live-verify-retry: ${passed} passed, ${failed} failed`);
