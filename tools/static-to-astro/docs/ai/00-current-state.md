...
**G-20u45 Schedule HTTP dry-run + Save source (2026-07-17):** **source complete · Edge Save not deployed** — `gosaki-schedule-save-dry-run` · dryRun STG verified · `operation=save` edit UPDATE + create INSERT · edit **published** allowed · edit **date** forbidden · approval `gosaki-schedule-operational-save` · user JWT + `is_admin` · create `published=false` + Edge `legacy_id` · STG Save default disabled · local arm `PUBLIC_GOSAKI_SCHEDULE_SAVE_UI_ARMED` · DB permission **unconfirmed** (live SELECT required) · **EDGE_DEPLOY_EXECUTED: false** · **SAVE_REQUEST_EXECUTED: false** · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md`. Verifier: `verify:g20u39b4-…`. **Next:** Commit/Push → permission SELECT preflight → staging Edge deploy ×1 → package/FTP → controlled Save ×1 each.

**G-20u45-gosaki-schedule-operational-edit-ui-wiring Gosaki Schedule operational edit UI (2026-07-17):** **complete** (+ lock + create preview + network dry-run source) — STG ContentPanel · local+endpoint dry-run wiring · **SCHEDULE_SAVE_DEFAULT_DISABLED: true** · service_role **not used**. Doc: `gosaki-admin-multi-route-staging-package-prep.md` (§ G-20u45). Verifier: `verify:g20u39b4-…`.
