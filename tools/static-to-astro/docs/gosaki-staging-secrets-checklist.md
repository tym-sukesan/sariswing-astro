# gosaki Staging Secrets Checklist（Phase G-3）

**目的:** gosaki staging 検証・FTP apply における secrets 管理ルール  
**原則:** secret 実値を doc / Git / レポートに書かない。本番 secrets を staging 検証に使わない。

**Staging 固定値（G-2b 成功時点）**

| 項目 | 値 |
| --- | --- |
| 公開 URL | `https://yskcreate.weblike.jp/cms-kit-staging/gosaki/` |
| FTP server dir | `/cms-kit-staging/gosaki/` |
| FTP server（推奨） | `ftp.lolipop.jp` |

関連: [gosaki-staging-runbook.md](./gosaki-staging-runbook.md) / [gosaki-staging-operations.md](./gosaki-staging-operations.md)
...
### Staging FTP（Phase G-2b apply 済み — G-3 運用）

`.env.local` に **staging 専用** のみ設定:
...
| ルール | 理由 |
| --- | --- |
| `ftp.lolipop.jp` を推奨 | `ftp.yskcreate.weblike.jp` は FTPS 証明書不一致で失敗することがある |
| パスワードをログに出さない | apply 失敗時も stderr をそのまま共有しない |
| `530 Login incorrect` 後は rotate | 一度露出疑いがあれば FTP パスワードを必ず変更 |
| `GOSAKI_PROD_FTP_*` を置かない | safety verifier が fail |

apply 前: `verify-staging-ftp-safety.mjs` + `verify-gosaki-readiness.mjs`（両方 PASS）  
apply: [gosaki-staging-runbook.md](./gosaki-staging-runbook.md) §3-4

**manifest 記録（secret なし）:** `stagingNoindex`, `robotsDisallowAll`, `canonicalMode`, `productionIndexable`
...
### Storage asset plan（Phase 3-U dry-run）

readiness では export 済み JSON（`generated-astro/src/data`）を `--data-dir` で読む。**Storage upload はしない。**

- `uploads performed: no` を維持
- manifest / report に credentials を含めない（`secret leak: none`）
- `output/supabase-seed/gosaki` が無くても `--data-dir` があれば plan 可能
...
Phase G-3: secrets checklist。**FTP パスワード・Supabase keys の実値は doc に記載していません。**
