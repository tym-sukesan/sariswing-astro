...
import { G6G1_SCHEDULE_TITLE_NON_DRY_RUN_SLICE_APPROVAL_ID, G6G2_SCHEDULE_TIME_FIELDS_NON_DRY_RUN_SLICE_APPROVAL_ID } from "./schedule-write-types";
...
// --- G-6-g2 time fields slice ---

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_PHASE =
  "G-6-g2-schedule-time-fields-non-dry-run-slice-implementation";

export const G6G2_SCHEDULE_TIME_FIELDS_DRY_RUN_PREVIEW_APPROVAL_ID =
  "G-6-g2-schedule-time-fields-dry-run-preview";

export const SCHEDULE_G6G2_TIME_FIELDS_NON_DRY_RUN_ARMED_ENV =
  "PUBLIC_ADMIN_SCHEDULE_G6G2_TIME_FIELDS_NON_DRY_RUN_ARMED";

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_TARGET_ID =
  SCHEDULE_NON_DRY_RUN_POC_TARGET_ID;

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_LEGACY_ID = "schedule-2026-07-010";

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_DEFAULT_OPEN_TIME =
  "[CMS Kit staging] G-6-g2 open PoC";

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_DEFAULT_START_TIME =
  "[CMS Kit staging] G-6-g2 start PoC";

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_EXPECTED_TITLE =
  "[CMS Kit staging] G-6-g1 title PoC";

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_EXPECTED_VENUE =
  G6G1_SCHEDULE_TITLE_SLICE_EXPECTED_VENUE;

export const G6G2_SCHEDULE_TIME_FIELDS_SLICE_EXPECTED_DESCRIPTION =
  G6G1_SCHEDULE_TITLE_SLICE_EXPECTED_DESCRIPTION;

export interface ScheduleG6G2TimeFieldsEditConfig {
  phase: typeof G6G2_SCHEDULE_TIME_FIELDS_SLICE_PHASE;
  approvalId: typeof G6G2_SCHEDULE_TIME_FIELDS_NON_DRY_RUN_SLICE_APPROVAL_ID;
  dryRunPreviewApprovalId: typeof G6G2_SCHEDULE_TIME_FIELDS_DRY_RUN_PREVIEW_APPROVAL_ID;
  targetId: typeof G6G2_SCHEDULE_TIME_FIELDS_SLICE_TARGET_ID;
  sectionVisible: boolean;
  previewEnabled: boolean;
  armed: boolean;
  saveEnabled: boolean;
  ...
  expectedProject: string;
  expectedSupabaseHost: string;
  g6g1ArmConflict: boolean;
}

export function getScheduleG6G2TimeFieldsEditConfig(
  env: ImportMetaEnv = import.meta.env,
): ScheduleG6G2TimeFieldsEditConfig {
  const mergedEnv = mergeStagingShellEnv(env);
  const dev = mergedEnv.DEV === true;
  const stagingShellEnabled = mergedEnv.ENABLE_ADMIN_STAGING_SHELL === "true";
  const stagingWriteFlag = mergedEnv.ENABLE_ADMIN_STAGING_WRITE === "true";
  ...
  const moduleMatch = module === "schedule";
  const approvalIdMatch =
    approvalIdEnv === G6G2_SCHEDULE_TIME_FIELDS_NON_DRY_RUN_SLICE_APPROVAL_ID;
  const dryRunFlagMatch = dryRun === false;
  const targetIdMatch =
    targetIdEnv === G6G2_SCHEDULE_TIME_FIELDS_SLICE_TARGET_ID || targetIdEnv === "";

  const base = {
    phase: G6G2_SCHEDULE_TIME_FIELDS_SLICE_PHASE,
    approvalId: G6G2_SCHEDULE_TIME_FIELDS_NON_DRY_RUN_SLICE_APPROVAL_ID,
    dryRunPreviewApprovalId: G6G2_SCHEDULE_TIME_FIELDS_DRY_RUN_PREVIEW_APPROVAL_ID,
    targetId: G6G2_SCHEDULE_TIME_FIELDS_SLICE_TARGET_ID,
    dryRun,
    serviceRoleAllowed: false as const,
    usesAuthenticatedUserSession: true as const,
    manualConfirmRequired: true as const,
    timeFieldsOnlyPayload: true as const,
    dev,
    stagingShellEnabled,
    stagingWriteFlag,
    providerMatch,
  ...
  if (!approvalIdMatch) {
    armFailures.push(
      `PUBLIC_ADMIN_WRITE_APPROVAL_ID=${G6G2_SCHEDULE_TIME_FIELDS_NON_DRY_RUN_SLICE_APPROVAL_ID}`,
    );
  }
  if (!dryRunFlagMatch) armFailures.push("PUBLIC_ADMIN_WRITE_DRY_RUN=false");
  if (!armedFlagMatch) {
    armFailures.push(`${SCHEDULE_G6G2_TIME_FIELDS_NON_DRY_RUN_ARMED_ENV}=true`);
  }
  if (g6g1Armed) {
  ...
  if (targetIdEnv && !targetIdMatch) {
    armFailures.push(
      `${SCHEDULE_SLICE_TARGET_ID_ENV}=${G6G2_SCHEDULE_TIME_FIELDS_SLICE_TARGET_ID}`,
    );
  }
  if (!supabaseConfigured) armFailures.push("Supabase URL/anon key");
  ...
  return {
    ...
    armed,
    saveEnabled: armed,
    disabledReason: previewEnabled ? undefined : "G-6-g2 section requires DEV + staging shell.",
    armFailureReason: armed ? undefined : armFailures.join("; "),
  };
}
