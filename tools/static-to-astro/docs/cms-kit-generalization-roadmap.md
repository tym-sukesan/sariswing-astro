...
**G-6-d-prep（完了）:** [staging-profile-update-poc-approval-plan.md](./staging-profile-update-poc-approval-plan.md) — planning / approval only; approval ID `G-6-d-staging-profile-update-poc`; `canWrite: false`; Save button remains disabled; `readyForG6DApproval: true`。次: **G-6-d** staging profile update PoC with explicit approval only。
