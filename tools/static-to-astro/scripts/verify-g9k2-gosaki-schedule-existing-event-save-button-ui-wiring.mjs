...
assert("UI does not import update-save module", !uiSrc.includes("gosaki-schedule-existing-event-update-save"));
...
assert("UI save readiness message", uiSrc.includes("保存準備OK。ただし G-9k2 では実保存未開放"));
assert("UI G-9k4 not enabled message", uiSrc.includes("G-9k4"));
...
assert("UI update button safe stop", uiSrc.includes("gosaki-schedule-update-btn"));
...
assert("G-9k4 copy in Astro", astroSrc.includes("G-9k4"));
...
for (const script of [
  "verify-g9k-gosaki-schedule-existing-event-save-button-enablement-planning.mjs",
  "verify-g9k1-gosaki-schedule-existing-event-save-button-guard-config.mjs",
]) {
