...
**G-5z-c（完了）:** [supabase-read-only-data-adapter.md](./supabase-read-only-data-adapter.md) — env-gated Supabase read-only adapter。approval `G-5z-c-staging-read-only-data-connect`。次: **G-5z-d**。
