...
**CMS Core v2 About Save UI armed package verify mode (2026-07-28):** **COMPLETE (local verifier only)** — default `verify:manual-upload` still expects `aboutSaveUiArmed=false` · armed packages require explicit `--expect-about-save-ui-armed` / `npm run verify:manual-upload:about-save-ui-armed` · HTML+PACKAGE_RUN cross-check · Contents path endpoints rejected for About · Save/FTP/remote arm **not** run · **Next:** use armed verify on current Save-UI-armed package then operator FTP when approved.

**CMS Core v2 About Supabase profile.lede Save client adapter (2026-07-28):** **COMPLETE (local only)** — supabase dry-run/Save sanitize · gate accepts supabase approval · Edit lock without fileSha · Contents path unchanged · Save UI default false · Doc: `cms-core-v2-about-supabase-profile-lede-save-client-adapter-implementation.md` · Verifier: `verify-cms-core-v2-about-supabase-save-client-adapter.mjs` · Gates: `ABOUT_SUPABASE_PROFILE_LEDE_SAVE_CLIENT_ADAPTER_IMPLEMENTATION_COMPLETE: true` · `readyForAboutSupabaseProfileLedeSaveArmedPackageGenerate: true` · `SAVE_ARM_ENABLED: false` · **Next:** superseded by armed package verify mode above when package already generated.

**CMS Core v2 About Supabase profile.lede Save roundtrip planning (2026-07-28):** **COMPLETE** — planning + adapter implementation above · Doc: `cms-core-v2-about-supabase-profile-lede-save-roundtrip-planning.md`.
