...
**G-6-d-hardening（完了）:** [staging-profile-write-hardening.md](./staging-profile-write-hardening.md) — `updated_by` NULL / UI role mock-only 整理; dry-run 復帰手順; `readyForG6EPlanning: true`; `readyForG6EImplementation: false`; 次は RLS audit / G-6-e-planning。
