-- G-20u36b-edge-dry-run-endpoint-deploy-preflight — Gosaki Discography Edge deploy preflight
...
--
-- Context: Before Edge Function gosaki-discography-save-dry-run deploy (dryRun only)
-- Purpose: Verify grants · RLS · data baseline before deploy-manual phase
-- Cursor does NOT execute this file. Operator confirms project ref before run.
-- No REVOKE / GRANT / RLS policy changes · no Edge deploy in this phase.
...
WITH params AS (
  SELECT
    ...
    'gosaki-discography-save-dry-run'::text AS endpoint_name,
...
checks AS (
  ...
  SELECT
    'A.endpoint.deploy_target',
    'INFO',
    'gosaki-discography-save-dry-run',
    vf.endpoint_name,
    jsonb_build_object(
      'operation', 'dryRun',
      'site_slug', vf.target_site_slug,
      'initial_legacy_id', vf.target_legacy_id,
      'staging_url_pattern', 'https://kmjqppxjdnwwrtaeqjta.supabase.co/functions/v1/gosaki-discography-save-dry-run',
      'save_rejected', true,
      'didWrite_always_false', true,
      'dbWrite_always_false', true,
      'networkWrite_always_false', true
    )
  FROM verify_flags vf
  ...
  SELECT
    ...
    'INFO',
    'may remain — grant layer blocks direct UPDATE',
  ...
  SELECT
    'F.effective_write_risk.before_edge_deploy',
    CASE
      ...
      ELSE 'PASS'
    END,
    'no longer RISK — authenticated UPDATE grant = 0',
    ...
    jsonb_build_object(
      ...
      'edge_dry_run_didWrite_always_false', true
    )
  FROM verify_flags vf
  ...
  SELECT
    'G.permissions_gate.deploy_preflight',
    CASE
      ...
      ELSE 'STOP'
    END,
    'permissions remediation baseline intact',
    CASE
      WHEN vf.flag_no_auth_update
        AND vf.flag_anon_write_zero
        AND vf.flag_auth_select_present
    ...
    jsonb_build_object(
      ...
      'admin_fetch_post_not_added', true,
      'edge_deploy_not_executed_in_this_phase', true
    )
  FROM verify_flags vf
  ...
  SELECT
    ...
    CASE
      ...
      WHEN vf.authenticated_update_count > 0 THEN 'STOP'
      ELSE 'READY_FOR_EDGE_DRY_RUN_DEPLOY_PREFLIGHT'
    END,
    'READY_FOR_EDGE_DRY_RUN_DEPLOY_PREFLIGHT or STOP',
    CASE
      WHEN NOT (
        vf.flag_no_auth_update
        AND vf.flag_anon_write_zero
      ...
      WHEN vf.authenticated_update_count > 0 THEN 'STOP — authenticated UPDATE grants remain'
      ELSE 'READY_FOR_EDGE_DRY_RUN_DEPLOY_PREFLIGHT — operator may record result; deploy-manual is separate approval'
    END,
    jsonb_build_object(
      'effective_write_risk', vf.effective_write_risk_after,
      'endpoint_name', vf.endpoint_name,
      'next_phase', 'G-20u36b-edge-dry-run-endpoint-deploy-preflight-result-record',
      'deploy_manual_blocked_until', 'preflight-result-record PASS + operator approval'
    )
  FROM verify_flags vf
)
SELECT
