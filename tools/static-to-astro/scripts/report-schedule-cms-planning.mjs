#!/usr/bin/env node
/**
 * G-6-e-planning-schedule-cms — Report CLI (planning only; no DB access).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runScheduleCmsPlanningReport,
  writeScheduleCmsPlanningOutput,
} from "./lib/schedule-cms-planning-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      `Usage: node scripts/report-schedule-cms-planning.mjs [--out-dir PATH]`,
    );
    process.exit(0);
  }
  ...
  const report = runScheduleCmsPlanningReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-schedule-cms-planning");
  console.log(`  phase: ${report.phase}`);
  console.log(`  planningOnly: ${report.planningOnly}`);
  console.log(`  targetModule: ${report.targetModule}`);
  console.log(`  derivedFrom: ${report.derivedFrom}`);
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeScheduleCmsPlanningOutput(absOut, report);
  ...
  console.log("OK — schedule CMS planning report complete.");
}
