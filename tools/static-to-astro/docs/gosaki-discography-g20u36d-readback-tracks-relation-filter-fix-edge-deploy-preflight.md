# G-20u36d — Gosaki Discography Edge dry-run readBack tracks relation filter fix edge deploy preflight
...
**Date:** 2026-07-13  
**Base commit:** `4aba597`  
**Prior:** G-20u36d readBack tracks relation filter fix root placement · live endpoint still pre-fix (`release_id` filter on deployed Edge)
...
```txt
gosakiDiscographyEdgeDryRunReadBackTracksRelationFilterFixEdgeDeployPreflightReady: true
phase: G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight
preflightOnly: true
...
**G-20u36d readBack tracks-relation-filter-fix edge-deploy-preflight scope:** preflight doc + verifier only. No deploy, no SQL, no Save enablement, no admin UI change, no root function edit.

---

## Why a new preflight (context)

After tracks relation filter fix root placement:

| Verifier | Result |
| --- | --- |
| `verify:g20u36d-readback-tracks-relation-filter-fix-root-placement` | **PASS** |
| `verify:g20u36d-readback-tracks-relation-filter-fix-tools-draft` | **PASS** |
| `verify:g20u36d-readback-tracks-relation-filter-fix-plan` | **PASS** |
| `verify:g20u36d-readback-tracks-select-fields-fix-edge-deploy-preflight` (historical) | **not this deploy gate** — expects pre-relation-fix `releaseRow.id` tracks path |
| `verify:g20u36d-readback-release-id-select-fix-edge-deploy-preflight` (historical) | **not this deploy gate** — prior release-id redeploy only |

**Deploy gate decision:** use **this tracks-relation-filter-fix preflight** for the relation filter redeploy. Live verify retry-2 showed deployed Edge still filters tracks by **`release_id`** (column absent) → anon SELECT **400** → `trackCount=0`. Root source now uses **`discography_legacy_id=eq.{legacyId}`**.
...
| **Production STOP** | `vsbvndwuajjhnzpohghh` — **forbidden / unused** |
| **Deploy type** | **Redeploy** — `release_id` filter deployed Edge → `discography_legacy_id` filter root source |
...
| Only `supabase/.temp/cli-latest` changed | **restore** that file before commit (same as prior G-20u36d deploy-result phases) |

---

## Root source safety (tracks relation filter fix — read-only review)

| Check | Expected |
| --- | --- |
| Tracks filter | **`site_slug=eq.{siteSlug}`** + **`discography_legacy_id=eq.{legacyId}`** |
| `release_id=eq.{uuid}` filter | **removed** |
| `fetchTracks` input | `{ siteSlug, legacyId }` |
| `releaseRow.id` / UUID for tracks relation | **not used** |
| `TRACK_SELECT_FIELDS` | **`track_number, title, sort_order, site_slug`** — no **`duration`** / **`release_id`** |
| `discography_legacy_id` in SELECT list | **no** — filter parameter only |
...
| Verifier | Purpose |
| --- | --- |
| `verify:g20u36d-readback-tracks-relation-filter-fix-edge-deploy-preflight` | This preflight doc + gates |
| `verify:g20u36d-readback-tracks-relation-filter-fix-root-placement` | Relation filter fix root source placed |
| `verify:g20u36d-readback-tracks-relation-filter-fix-tools-draft` | Tools draft / mock baseline |
| `verify:g20u36d-readback-env-secret-setting-result` | readBack env secret added |

All targeted verifiers above must **PASS** before operator proceeds to **G-20u36d-readback-tracks-relation-filter-fix-edge-deploy**.

**Do not use** historical `verify:g20u36d-readback-tracks-select-fields-fix-edge-deploy-preflight` or `verify:g20u36d-readback-release-id-select-fix-edge-deploy-preflight` as this relation-filter redeploy gate — they reference pre-relation-fix root handler expectations.

---

## Deploy execution readiness decision

**Proceed to G-20u36d-readback-tracks-relation-filter-fix-edge-deploy when ALL are true:**
...
| 10 | Root tracks path uses **`discography_legacy_id=eq.{legacyId}`** · **no `release_id=eq.`** |

**If any criterion fails → STOP** (see STOP conditions below).

---

## Post-deploy expectations (live verify retry-3 — NOT this phase)

| Scenario | Expected (readBack armed · relation filter fix deployed) |
| --- | --- |
...
**Save planning:** **G-20u36e-controlled-save-planning** — only after live verify retry-3 **PASS**.
...
| Root tracks path still uses `release_id=eq.` | **STOP** |
| Root tracks path missing `discography_legacy_id=eq.` | **STOP** |
| `releaseRow.id` still used for tracks relation | **STOP** |
...
| Live HTTP verify retry-3 | **not executed** |
...
| Phase | Scope |
| --- | --- |
| **G-20u36d-readback-tracks-relation-filter-fix-edge-deploy** | Operator staging Edge redeploy |
| **G-20u36d-readback-tracks-relation-filter-fix-edge-deploy-result-record** | Deploy result doc |
