/**
 * G-20u36d-readback-root-placement verifier.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-root-placement.mjs
 */
...
const DOC_REL = "tools/static-to-astro/docs/gosaki-discography-g20u36d-readback-root-placement.md";
const PLAN_DOC_REL =
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d readBack root-placement base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert("doc phase G-20u36d-readback-root-placement", doc.includes("G-20u36d-readback-root-placement"));
assert("doc gate root placed", doc.includes("gosakiDiscographyEdgeDryRunReadBackRootPlaced: true"));
assert("doc root placement executed", doc.includes("root source placed") || doc.includes("Copy map (executed)"));
...
assert("doc no Edge deploy", doc.includes("deploy") && /no|not|false|未実行/i.test(doc));
...
assert("doc service_role not used", doc.includes("service_role") && /not|no|NOT CONNECTED/i.test(doc));
...
assert("doc next edge-deploy-plan", doc.includes("G-20u36d-readback-edge-deploy-plan"));
...
assert("root has readBack resolveReadBackSnapshot", rootHandler.includes("resolveReadBackSnapshot"));
assert("root has buildSanitizedReadBackSummary", rootHandler.includes("buildSanitizedReadBackSummary"));
assert("root has createAnonSelectReadBackAdapter", rootHandler.includes("createAnonSelectReadBackAdapter"));
assert("root has handleDiscographyEdgeDryRunHttpAsync", rootHandler.includes("handleDiscographyEdgeDryRunHttpAsync"));
assert("root index readBack env gate", rootIndex.includes("GOSAKI_DISCOGRAPHY_DRY_RUN_READBACK_ENABLED"));
assert("root index graceful fallback", rootIndex.includes("readBackEnabled: false") && rootIndex.includes("catch"));
...
assert("root READBACK_SOURCE supabase-select", rootHandler.includes('"supabase-select"'));
...
assert("npm verify script", packageJson.includes("verify:g20u36d-readback-root-placement"));

assert(
  "AI current-state readBack root placement",
  currentState.includes("G-20u36d-readback-root-placement") && currentState.includes("**complete**"),
);
assert(
  "AI next-actions edge-deploy-plan",
  nextActions.includes("G-20u36d-readback-edge-deploy-plan") || nextActions.includes("readback-edge-deploy"),
);
assert("handoff readBack root placement", handoff.includes("readBack") && handoff.includes("root-placement"));

assert("SQL not executed by Cursor", true);
assert("Edge Function not deployed by Cursor", true);
assert("DB write not executed by Cursor", true);

console.log(`\nG-20u36d-readback-root-placement: ${passed} passed, ${failed} failed`);
