...
**Discography site-owner authz Slice A apply-packet atomicity hardening (2026-08-15):** **COMPLETE (offline templates)** — FORWARD/ROLLBACK RPC `BEGIN`/`COMMIT` wrap CREATE+COMMENT+REVOKE+GRANT · RLS comments aligned to catalog (no RESTRICTIVE policies present) · logic unchanged · `STAGING_APPLY_READY: true` · `STAGING_APPLY_EXECUTED: false` · apply still needs explicit human approval.
