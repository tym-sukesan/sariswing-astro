...
- **Staging migration gate:** `cms-core-v2-schedule-tbd-staging-migration-gate` **COMPLETE** · **final review COMPLETE** · **apply completion PASS (2026-08-02)** · staging `date` nullable + `date_status` · runtime SELECT still omits `date_status`
- **Seed still blocked:** regenerate SQL for TBD null + `date_status` before apply · `READY_FOR_MIO_SEED_APPLY: false`
