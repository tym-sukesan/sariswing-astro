...
2. **並行可 (Kit Core):** **Next Primary** `cms-core-v2-schedule-tbd-date-save-non-dry-run-staging-execution`（final-preflight COMPLETE · `EXECUTION_PACKET_READY: true` · `PREFLIGHT_PASS: false` until SQL Editor 79 · `ACTUAL_WRITE_READY: false`）→ operator SQL Editor → arm ON → Save once → arms OFF → Mio seed regen/apply → Branch A live SELECT pilot · then generic read-only Admin · form onboarding · Admin runtime Save arm mutex（**別承認**）· evening-set hero（**NON_BLOCKING**）。
...
## 0. CMS Core v2 Schedule TBD non-dry-run staging final-preflight (2026-08-03)
...
| Full table 79 | **pending operator SQL Editor** |
| Packet | runbook A–J · post-check · exact cleanup |
| Write | `ACTUAL_WRITE_READY: false` |
...
| Next | final-preflight COMPLETE → execution |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_SCHEDULE_TBD_DATE_STATUS_READ_WIRING_COMPLETE: true` |
...
| Anon (pre-wiring) | Gosaki 74 · normalize PASS |
| Next | superseded → date-status read-wiring above |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_SCHEDULE_TBD_DATE_ADMIN_SAVE_PLANNING_COMPLETE: true` |
...
## 0. CMS Core v2 Schedule TBD date contract planning (2026-08-01)

| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_SCHEDULE_TBD_DATE_CONTRACT_PLANNING_COMPLETE: true` |
| Recommended | nullable `date` + `date_status` (`confirmed`/`tbd`) |
| Sentinel | **REJECTED** |
...
| Next | Gosaki read compat → migration → seed regen/apply → Branch A |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_EXTERNAL_FORM_HUBSPOT_COMPLETION_AUDIT_COMPLETE: true` |
...
| PC/375 browser baseline | **COMPLETE / PASS** (display only; no submit) |
...
| Next Kit | `cms-core-v2-mio-supabase-live-select-only-pilot` |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_EXTERNAL_FORM_PROVIDER_HUBSPOT_SHADOW_COMPARE_COMPLETE: true` |
...
## 0. CMS Core v2 external form HubSpot pure renderer (2026-07-31)

| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_EXTERNAL_FORM_PROVIDER_HUBSPOT_RENDERER_COMPLETE: true` |
| Phase | `cms-core-v2-external-form-provider-hubspot-renderer` |
| Verdict | **COMPLETE** |
| API | `renderHubspotConfigHtml(validatedResult)` |
| Verifier | `verify:cms-core-v2-external-form-provider-hubspot-renderer` |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_EXTERNAL_FORM_PROVIDER_GOOGLE_FORMS_COMPLETE: true` |
...
| HubSpot renderer | **COMPLETE** (above) |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_MIO_ABOUT_READ_RENDER_COMPLETE: true` |
| Phase | `cms-core-v2-mio-about-read-render` |
| Helper | `scripts/lib/mio-about-data-page.mjs` |
| Inject | `aboutBundle` / `siteAboutBundle` (`buildMioInjectAboutBundle`) |
| Content | JA short/long · EN · photo + no-photo · collaborators ×3 |
| Verifier | `verify:cms-core-v2-mio-about-read-render` (88 PASS + safety-suite) |
| Contact / Admin / Save / package / FTP / DB | **false** |
...
| About read-render | **complete** (above) |
| Contact / Admin / Save / package / FTP / DB | **false** |
| Next | `cms-core-v2-mio-read-render-browser-baseline` |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_MIO_SCHEDULE_READ_RENDER_COMPLETE: true` |
...
| Next | `cms-core-v2-mio-read-render-browser-baseline` (about done) |
...
| Scope | Videos + footer + Schedule + Discography + About |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_MIO_DATA_FIXTURES_COMPLETE: true` |
...
## 0. CMS Core v2 Mio registry noop pilot (2026-07-31)
...
## 0. CMS Core v2 Mio static fixture scaffold (2026-07-31)
...
| Scaffold | **complete** (see above) |
| Next | `cms-core-v2-mio-registry-noop-pilot` |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_GOSAKI_SITE_GENERATOR_HOOKS_ADAPTER_COMPLETE: true` |
| Phase | `cms-core-v2-gosaki-site-generator-hooks-adapter` |
| Adapter | `gosaki-site-generator-hooks-adapter.mjs` (lazy via registry `generatorHooksAdapter`) |
| Core | `site-generator-hooks.mjs` — no `gosaki-*` imports; `astro-generator` no adapter import |
| Registration | `ensureSiteGeneratorHookAdapter` / `resolveSiteGeneratorHooksAsync` (idempotent) |
| HTML baseline | unchanged fixtures · deep-eq PASS |
| Behavior | **unchanged** |

```txt
CMS_CORE_V2_GOSAKI_SITE_GENERATOR_HOOKS_ADAPTER_COMPLETE: true
FACTORY_MOVED: true
...
| Factory moved | **true** (adapter phase) |
...
| Extract | audit then adapter move (follow-up) |
| Doc | `cms-core-v2-site-generator-hooks-boundary-audit.md` |
| Defer | ~~move factory~~ **done** in adapter phase |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_SITE_PACKAGE_VERIFIER_EXTENSION_DECOUPLING_COMPLETE: true` |
| Phase | `cms-core-v2-site-package-verifier-extension-decoupling` |
| Core | `verify-site-package-core` — optional `siteExtensionVerifier` |
...
| Behavior | **unchanged** (Gosaki entrypoints inject) |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_RESIDUAL_CORE_TO_SITE_DEPENDENCY_AUDIT_COMPLETE: true` |
...
| Next candidate | trim-true PATH/BUILD_READ helper — **done** |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_NEXT_HIGH_VALUE_SAFE_EXTRACTION_COMPLETE: true` |
| Phase | `cms-core-v2-next-high-value-safe-extraction` |
| Extraction | `supabase-anon-read-env-utils` (`resolveSupabaseAnonReadEnv` + `loadDotEnvLocal`) |
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_GLOBAL_SAVE_ARM_MUTEX_PACKAGE_GATE_COMPLETE: true` |
| Phase | `cms-core-v2-global-save-arm-mutex-package-gate` |
| Adapter | `scripts/lib/gosaki-operational-save-ui-arm-mutex-gate.mjs` |
| Wire | `runSitePackageBuild` before relocate · `createManualUploadPackage` before rm/mkdir |
| npm | `verify:cms-core-v2-global-save-arm-mutex-package-gate` |
...
## 0. CMS Core v2 global Save arm mutex helper (2026-07-30)

| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_GLOBAL_SAVE_ARM_MUTEX_HELPER_COMPLETE: true` |
...
| MUTEX_EVALUATOR_AVAILABLE | **true** |
| MUTEX_EVALUATOR_WIRED | **true** (via package gate) |
| PACKAGE_GENERATE_GATE_WIRED | **true** |
| GLOBAL_MULTI_ARM_MUTEX_IMPLEMENTED | **true** |
...
| GLOBAL_MULTI_ARM_MUTEX_IMPLEMENTED | **true** |
| Evaluator / package gate | **wired** (package generate · Admin runtime still false) |
...
| Code / helper / verifier (policy phase) | docs-only at policy time · inventory + helper + package gate now landed (§14–§16) |
...
| GLOBAL_MULTI_ARM_MUTEX_IMPLEMENTED | **true** (package generate) |
...
## 0. CMS Core v2 Save arm parse policy verifier (2026-07-29)
...
| Item | Value |
| --- | --- |
| Gate | `CMS_CORE_V2_ADMIN_STAGING_REF_ASSERT_HELPER_COMPLETE: true` |
| Core SoT | `scripts/lib/supabase-staging-ref-utils.mjs` |
| Edge/Deno wired | **false** |
| Admin UI changed | **false** |
| CLIENT_SHARE_READY | **true**（維持） |
| Deployed package | `dc1c5b6…` **unchanged** |
| Doc | `cms-core-v2-admin-staging-ref-assert-helper.md` |
| Verifier | `npm run verify:cms-core-v2-admin-staging-ref` |
...
| Item | Value |
| --- | --- |
| Gate | `ABOUT_SUPABASE_PROFILE_LEDE_SAVE_ROUNDTRIP_PASSED: true` |
| Impl commit | `359a04a3e960d23fe4948ee53cab7580b924f86f` |
| Verifier / package commit | `fe4732b1552934f83ebd7cbbb2b49ef98da95aaa` |
| Forward / restore Save | **PASS** · DB final = baseline |
| Remote arm final | `GOSAKI_ABOUT_SUPABASE_SAVE_ARMED=false` |
| Live package | disarmed · `verify:manual-upload` PASS · FTP done |
| Next readiness | `readyForAboutSupabasePublicBuildReadPlanning: true` |
...
| Commit | `359a04a3e960d23fe4948ee53cab7580b924f86f` |
...
| Execution | **PASS** — see result section above |
...
| Item | Value |
| --- | --- |
| Gate | `ABOUT_SUPABASE_ADMIN_PATH_PACKAGE_PREFLIGHT_COMPLETE: true` |
| Generate readiness | `readyForAboutSupabaseAdminPathPackageGenerate: true` |
| Package / FTP | **not executed** |
| Path env | `PUBLIC_ADMIN_GOSAKI_ABOUT_SUPABASE_PATH_ENABLED=true` (bake) |
| UI Save arm | `PUBLIC_ADMIN_GOSAKI_ABOUT_SUPABASE_SAVE_UI_ARMED=false` |
| Server Save arm | unset |
| Doc | `cms-core-v2-about-supabase-admin-path-package-preflight.md` |
| Verifier | `verify-cms-core-v2-about-supabase-admin-path-package-preflight.mjs` |
...
## 0. CMS Core v2 About Supabase Edge post-deploy QA COMPLETE (2026-07-27)

| Item | Value |
| --- | --- |
| Gate | `POST_DEPLOY_QA_PASSED: true` |
| Doc | `cms-core-v2-about-supabase-save-dry-run-edge-post-deploy-qa-result.md` |
...
| Status | confirmed by remote QA (`ok:false`) |
...
| Item | Value |
| --- | --- |
| Gate | `rlsServiceRoleRevokeHarden: true` |
| `readyForOperatorAboutRlsApply` | **true** (operator re-accepted; harden retained) |
| `readyForOperatorAboutMigrationApply` | **false** (HOLD / already applied) |
| Change | RLS `REVOKE ALL … FROM service_role` + post-RLS privilege SELECT |
| Policies / anon·auth GRANTs | **unchanged** |
| Doc | `cms-core-v2-about-supabase-vertical-slice-apply-readiness.md` |
...
| `readyForOperatorAboutMigrationApply` | **false** (migration applied; RLS path) |
...
| `readyForOperatorAboutMigrationApply` | **false** (RLS HOLD; harden retained) |
...
| Apply | **`readyForOperatorAboutRlsApply: true`** · **`readyForOperatorAboutMigrationApply: false`** (migration applied; RLS ready) |
