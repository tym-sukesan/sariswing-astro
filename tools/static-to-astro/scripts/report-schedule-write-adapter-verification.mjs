#!/usr/bin/env node
/**
 * G-6-e4-schedule-write-adapter-verification — Report CLI (static scan; no DB access).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runScheduleWriteAdapterVerificationReport,
  writeScheduleWriteAdapterVerificationOutput,
} from "./lib/schedule-write-adapter-verification-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      `Usage: node scripts/report-schedule-write-adapter-verification.mjs [--out-dir PATH]`,
    );
    process.exit(0);
  }
  ...
  const report = runScheduleWriteAdapterVerificationReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-schedule-write-adapter-verification");
  console.log(`  phase: ${report.phase}`);
  console.log(`  writeAdapterImplemented: ${report.writeAdapterImplemented}`);
  console.log(`  writeAdapterVerified: ${report.writeAdapterVerified}`);
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeScheduleWriteAdapterVerificationOutput(
      absOut,
      report,
    );
  ...
  console.log("OK — schedule write adapter verification report complete.");
}
