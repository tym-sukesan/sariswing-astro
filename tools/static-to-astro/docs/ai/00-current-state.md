...
**CMS Core v2 About Supabase Admin read/hydrate planning (2026-07-28):** **COMPLETE (docs only · no impl)** — dedicated Admin `operation=read` for latest `site_page_fields.profile.lede` · surgical lede overlay · JSON fallback · Save arms false · public About JSON unchanged · Contents retained · Doc: `cms-core-v2-about-supabase-admin-read-hydrate-planning.md` · Gates: `ABOUT_SUPABASE_ADMIN_READ_HYDRATE_PLANNING_COMPLETE: true` · `readyForAboutSupabaseAdminReadHydrateImplementation: true` · `IMPLEMENTATION_EXECUTED: false` · **Next:** ask operator → local read/hydrate implementation (no Edge deploy / FTP / Save arm until separate phases).

**CMS Core v2 About Supabase Admin-path staging FTP post-QA (2026-07-28):** **COMPLETE / PASS** — package `sourceCommit` **`a876e1ebd4523d96b09d1ea46fd35748de27977e`** · build / static-public / `verify:manual-upload` PASS · FileZilla full `public-dist/` → staging · writeBackend **supabase** · saveDisabled **true** · `valueTextRequiredVisible: false` · remote Save arm **unset** · DB write **none** · public About **JSON** · stale-backup + external `PACKAGE_RUN` **worked** · production untouched · Doc: `cms-core-v2-about-supabase-admin-path-staging-ftp-post-qa-result.md` · ftp-post-qa §B **PASS** · Gates: `ABOUT_SUPABASE_ADMIN_PATH_STAGING_FTP_POST_QA_PASSED: true` · `readyForAboutSupabaseAdminReadHydratePlanning: true`.
