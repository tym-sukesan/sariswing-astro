...
**Status:** G-5a〜**G-5m-b（Admin Auth abstraction scaffold）** 完了 — G-5n 以降は段階的に  
...
- [admin-auth-abstraction-scaffold.md](./admin-auth-abstraction-scaffold.md) — **G-5m-b** Admin Auth abstraction
...
**G-5m-b（完了）:** [admin-auth-abstraction-scaffold.md](./admin-auth-abstraction-scaffold.md) — Auth UI + permissions draft。Supabase Auth 未接続。次: **G-5n** Media upload。
