/**
 * G-20u36d-readback-edge-deploy-result verifier.
 * Result-record only — no re-deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-edge-deploy-result.mjs
 */
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d readBack edge-deploy-result base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert(
  "doc phase G-20u36d-readback-edge-deploy-result-record",
  doc.includes("G-20u36d-readback-edge-deploy-result-record"),
);
assert("doc gate edge deployed", doc.includes("gosakiDiscographyEdgeDryRunReadBackEdgeDeployed: true"));
...
assert(
  "doc cli-latest restore",
  doc.includes("cli-latest") && /git restore|restored/i.test(doc),
);
assert(
  "doc final git clean",
...
assert("doc readBack capable deployed", doc.includes("readBack-capable") || doc.includes("readBackCapableCodeDeployed"));
assert("doc live verify not executed", doc.includes("live-verify") || doc.includes("Live verify") || doc.includes("liveVerifyExecuted: false"));
...
assert(
  "package script verify:g20u36d-readback-edge-deploy-result",
  packageJson.includes("verify:g20u36d-readback-edge-deploy-result"),
);
...
assert(
  ...
  currentState.includes("G-20u36d-readback-edge-deploy-result") ||
    currentState.includes("edge-deploy-result"),
);
assert("AI next-actions live-verify", nextActions.includes("G-20u36d-readback-live-verify"));
assert(
...
console.log(`\nG-20u36d-readback-edge-deploy-result: ${passed} passed, ${failed} failed`);
