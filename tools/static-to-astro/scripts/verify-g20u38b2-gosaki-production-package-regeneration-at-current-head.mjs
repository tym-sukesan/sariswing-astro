/**
 * G-20u38b2 Gosaki production package regeneration at current HEAD verifier.
...
const PHASE = "G-20u38b2-gosaki-production-package-regeneration-at-current-head";
const GATE = "gosakiProductionPackageRegeneratedAtCurrentHead: true";
const NEXT = "G-20u38c-gosaki-production-package-verification-review";
const GENERATION_HEAD = "1c1fb97";
...
assert("prior G-20i3 drift doc exists", exists(PRIOR_DOC_REL));
...
assert(
  "production package regenerated",
  /Production package regenerated.*yes|productionPackageRegenerated:\s*true/i.test(doc),
);
assert(
...
assert(
  "PRODUCTION_PACKAGE_FRESH_AT_GENERATION_HEAD true",
  /PRODUCTION_PACKAGE_FRESH_AT_GENERATION_HEAD:\s*true|productionPackageFreshAtGenerationHead:\s*true/i.test(
    doc,
  ),
);
assert(
...
assert(
  "G20I3 drift resolved",
  /G20I3_VERIFIER_DRIFT_RESOLVED:\s*true|g20i3VerifierDriftResolved:\s*true/i.test(doc),
);
assert(
  "pipeline exit 0 documented",
  /Pipeline exit.*\*\*0\*\*|pipeline exit.*0/i.test(doc),
);
assert(
  "G-20i3 74/74 documented",
  /74\/74/i.test(doc),
);
assert(
  "package stale after commit documented",
  /stale|packageStaleAfterResultCommit/i.test(doc),
);
assert(
  "no FTP upload",
...
assert("generation HEAD 1c1fb97", doc.includes(GENERATION_HEAD));
...
assert("admin exclusion section", /Admin exclusion/i.test(doc));
assert(
...
assert(
  "manifest sourceCommit matches generation HEAD",
  manifest.sourceCommit.startsWith(GENERATION_HEAD),
  `${manifest.sourceCommit} vs ${GENERATION_HEAD}`,
);
if (manifest.sourceCommit !== head) {
  console.log(
    `NOTE on-disk prod sourceCommit ${manifest.sourceCommit.slice(0, 7)} stale vs current HEAD ${head.slice(0, 7)} — expected after result commit`,
  );
} else {
  assert("manifest sourceCommit matches current HEAD", manifest.sourceCommit === head);
}
...
assert("admin dir absent", !fs.existsSync(path.join(PUBLIC_DIST, "admin")));
...
assert(
  "prior G-20i3 drift resolved gate",
  priorDoc.includes("g20i3VerifierDriftResolved: true") ||
    priorDoc.includes("G20I3_VERIFIER_DRIFT_RESOLVED"),
);
assert(
  "package.json verify script",
  packageJson.includes(
    "verify:g20u38b2-gosaki-production-package-regeneration-at-current-head",
  ),
);
assert(
  "00-current-state mentions G-20u38b2",
  currentState.includes("G-20u38b2") || currentState.includes(PHASE),
);
assert(
  "03-next-actions mentions G-20u38b2",
  nextActions.includes("G-20u38b2") || nextActions.includes(PHASE),
);
assert(
  "handoff mentions G-20u38b2",
  handoff.includes("G-20u38b2") || handoff.includes(PHASE),
);

console.log(
  `\nverify-g20u38b2-gosaki-production-package-regeneration-at-current-head: ${passed} passed, ${failed} failed`,
);
