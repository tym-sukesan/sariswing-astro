/**
 * G-20u36d-readback-release-id-select-fix-edge-deploy-result verifier.
 * Result-record only — no re-deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-release-id-select-fix-edge-deploy-result.mjs
 */
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d release-id edge-deploy-result base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert(
  "doc phase G-20u36d-readback-release-id-select-fix-edge-deploy-result-record",
  doc.includes("G-20u36d-readback-release-id-select-fix-edge-deploy-result-record"),
);
assert(
  "doc gate edge deployed",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackReleaseIdSelectFixEdgeDeployed: true"),
);
...
assert("doc production ref STOP", doc.includes(PRODUCTION_REF) && /STOP|forbidden|not used|禁止/i.test(doc));
...
assert(
  "doc cli-latest unchanged",
  doc.includes("cli-latest") && (/unchanged|restore not required|restoreRequired: false/i.test(doc)),
);
assert(
  "doc final git clean",
...
assert(
  "doc release-id fix deployed",
  doc.includes("release-id fix") || doc.includes("releaseIdSelectFixDeployed"),
);
assert(
...
assert(
  "doc live verify retry not executed",
  doc.includes("live-verify-retry") ||
    doc.includes("Live verify retry") ||
    doc.includes("liveVerifyRetryExecuted: false"),
);
...
assert(
  "package script verify:g20u36d-readback-release-id-select-fix-edge-deploy-result",
  packageJson.includes("verify:g20u36d-readback-release-id-select-fix-edge-deploy-result"),
);
...
assert(
  ...
  currentState.includes("G-20u36d-readback-release-id-select-fix-edge-deploy-result") ||
    currentState.includes("release-id-select-fix-edge-deploy-result"),
);
assert(
  "AI next-actions live-verify-retry",
  nextActions.includes("G-20u36d-readback-live-verify-retry") ||
    nextActions.includes("live-verify-retry"),
);
assert(
  "AI handoff readBack edge deploy result",
  handoff.includes("release-id") &&
    (handoff.includes("edge-deploy-result") || handoff.includes("EdgeDeployed")),
);
...
console.log(
  `\nG-20u36d-readback-release-id-select-fix-edge-deploy-result: ${passed} passed, ${failed} failed`,
);
