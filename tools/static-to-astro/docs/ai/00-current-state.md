...
**Discography site-owner authz Slice A live Edge can_write_site probe dataset regression fix (2026-08-16):** **COMPLETE (packet correction · not executed)** — §3/§6 no longer use DOM `data-gosaki-supabase-*` (live fail `supabaseConfigFound=false`) · reuse PASS'd Vite `getStagingAuthConfig` + `getStagingSupabaseClient` from staging-preflight-result · terminal `PUBLIC_SUPABASE_*` not assumed exported · §3 while Secret OFF · §6 exact snippet prepared **before** Secret ON (clipboard / other window, not Console) · after ON: paste prepared snippet once, no doc open/search/copy · `discography-999` / unset OFF unchanged · Cursor must **not** execute · SoT: `discography-site-owner-authz-slice-a-live-can-write-site-probe-final-hardening.md`.

**Discography site-owner authz Slice A live Edge can_write_site probe final hardening (2026-08-16):** **COMPLETE (packet hardened · not executed)** — Phase `discography-site-owner-authz-slice-a-live-can-write-site-probe-final-hardening` · HEAD baseline `7d0434b5` · operator SoT updated by dataset regression fix above · Cursor must **not** execute · Doc: `discography-site-owner-authz-slice-a-live-can-write-site-probe-final-hardening.md`.

**Discography site-owner authz Slice A live Edge can_write_site probe execution preflight (2026-08-16):** **COMPLETE (packet locked · not executed)** — Phase `discography-site-owner-authz-slice-a-live-can-write-site-probe-execution-preflight` · HEAD `7d0434b5` · CLI **2.102.0** · VERSION **47** · albums **4** / tracks **34** / `discography-999=0` · Secret ON `secrets set …=true --project-ref kmjqppxjdnwwrtaeqjta` · Secret OFF `secrets unset … --project-ref kmjqppxjdnwwrtaeqjta` (unset, not `=false`) · one owner POST → expected `release_read_failed` · operator SoT superseded by final-hardening above · `READY_FOR_OPERATOR_PROBE: true` · Cursor must **not** execute · Doc: `discography-site-owner-authz-slice-a-live-can-write-site-probe-execution-preflight.md`.
