...
**CMS Core v2 next Kit feature comparison (2026-07-24):** **COMPLETE (docs-only)** — candidates Discography / About / Schedule / Contact / Bands compared read-only · **Recommended next:** **About → Supabase** (last Contents SoT; reuse `sites`/`site_members`/`platform_admins` + YouTube dual-path) · Schedule/Discography already Supabase (`registry` true) → Core work is `site_id`/membership harden later · minimal slice: one About scalar/block · dual-path · gated Save · JSON fallback · Doc: `cms-core-v2-next-kit-feature-comparison.md` · Verifier: `verify-cms-core-v2-next-kit-feature-comparison.mjs` · Gates: `cmsCoreV2NextKitFeatureComparisonComplete: true` · `readyForAboutSupabaseSlicePreflight: true` · `aboutSupabaseImplementationExecuted: false` · **no** implementation/DB/FTP/production · **Next:** `cms-core-v2-about-supabase-vertical-slice-preflight` · optional parallel Contents YouTube retire planning · Save arm false 維持。
