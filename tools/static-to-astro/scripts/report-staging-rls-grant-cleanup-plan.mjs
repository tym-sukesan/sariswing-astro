#!/usr/bin/env node
/**
 * G-6-rls-grant-cleanup-plan — Report CLI (plan only; no DB access).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runStagingRlsGrantCleanupPlanReport,
  writeGrantCleanupPlanOutput,
} from "./lib/staging-rls-grant-cleanup-plan-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      `Usage: node scripts/report-staging-rls-grant-cleanup-plan.mjs [--out-dir PATH]`,
    );
    process.exit(0);
  }
  ...
  const report = runStagingRlsGrantCleanupPlanReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-staging-rls-grant-cleanup-plan");
  console.log(`  phase: ${report.phase}`);
  console.log(`  stagingOnly: ${report.stagingOnly}`);
  console.log(`  planOnly: ${report.planOnly}`);
  console.log(`  cleanupPlanCreated: ${report.cleanupPlanCreated}`);
  console.log(`  cleanupExecuted: ${report.cleanupExecuted}`);
  console.log(`  rollbackExecuted: ${report.rollbackExecuted}`);
  console.log(`  targetGrantTypes: ${JSON.stringify(report.targetGrantTypes)}`);
  console.log(
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeGrantCleanupPlanOutput(absOut, report);
  ...
  console.log("OK — staging RLS grant cleanup plan report complete.");
}
