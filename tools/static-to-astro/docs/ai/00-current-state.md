...
**G-20u36d-readback-release-id-select-fix-planning Gosaki Discography Edge dry-run readBack release id SELECT fix plan (2026-07-12):** **complete** — plan only · STOP cause: `RELEASE_SELECT_FIELDS` omits internal `id` → trackCount=0 · fix: add `id` for tracks `release_id` lookup · strip from snapshot/summary/response; **no root/tools draft edit / Edge deploy / SQL / DB write / Save / admin UI / FTP** · service_role **not used** · anon SELECT maintained. Doc: `gosaki-discography-g20u36d-readback-release-id-select-fix-plan.md`. Verifier: `verify-g20u36d-readback-release-id-select-fix-plan.mjs`. **Next:** G-20u36d-readback-release-id-select-fix-tools-draft.
