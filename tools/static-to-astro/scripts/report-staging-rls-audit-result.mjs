#!/usr/bin/env node
/**
 * G-6-rls-audit-result — Report CLI (read-only result doc; no DB access).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runStagingRlsAuditResultReport,
  writeRlsAuditResultOutput,
} from "./lib/staging-rls-audit-result-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      `Usage: node scripts/report-staging-rls-audit-result.mjs [--out-dir PATH]`,
    );
    process.exit(0);
  }
  ...
  const report = runStagingRlsAuditResultReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-staging-rls-audit-result");
  ...
  console.log(`  profileStatus: ${report.profileStatus}`);
  console.log(`  adminUsersPolicyStatus: ${report.adminUsersPolicyStatus}`);
  console.log(`  grantStatus: ${report.grantStatus}`);
  console.log(`  safeForPlanning: ${report.safeForPlanning}`);
  console.log(
  ...
  console.log(`  policyChangesPerformed: ${report.policyChangesPerformed}`);
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } = writeRlsAuditResultOutput(absOut, report);
  ...
  console.log("OK — staging RLS audit result report complete.");
}
