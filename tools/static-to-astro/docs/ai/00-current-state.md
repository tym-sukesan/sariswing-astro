...
**CMS Core v2 About Supabase Admin read/hydrate Edge re-deploy preflight (2026-07-28):** **COMPLETE (docs only · no deploy)** — Function `gosaki-about-supabase-save-dry-run` · staging `kmjqppxjdnwwrtaeqjta` · baseline `c93f9e86…` · `operation:read` contract locked · Save arm unset · root↔mirror PASS · Doc: `cms-core-v2-about-supabase-admin-read-hydrate-edge-deploy-preflight.md` · Verifier: `verify-cms-core-v2-about-supabase-admin-read-hydrate-edge-deploy-preflight.mjs` · Gates: `ABOUT_SUPABASE_ADMIN_READ_HYDRATE_EDGE_DEPLOY_PREFLIGHT_COMPLETE: true` · `readyForAboutSupabaseAdminReadHydrateEdgeRedeployExecution: true` · `EDGE_REDEPLOY_EXECUTED: false` · **Next:** operator approval → one-shot staging re-deploy → post-deploy read QA.

**CMS Core v2 About Supabase Admin read/hydrate local implementation (2026-07-28):** **COMPLETE (local only · no Edge deploy)** — Edge `operation=read` SELECT-only · JWT+`can_write_site` · allowlist about/`profile.lede` · Admin overlay lede only · JSON fallback · Save arms false · public About JSON unchanged · root↔mirror byte match · Doc: `cms-core-v2-about-supabase-admin-read-hydrate-local-implementation.md` · **Next:** superseded by Edge re-deploy preflight above.
