...
**CMS Core v2 YouTube Supabase staging Save round-trip (2026-07-24):** **COMPLETE** — staging `kmjqppxjdnwwrtaeqjta` · owner · forward+restore Save PASS (`sort_order` 10→11→10) · final `sort_order=10` · `published=true` · `source_url` unchanged · `updated_at=2026-07-23 15:38:35.562674+00` · Save arm **false** · production **unchanged** · Contents path **default** (cutover未実施) · Gates: `cmsCoreV2YoutubeSupabaseStagingSaveRoundTripComplete: true` · `saveArmEnabled: false` · Doc: `cms-core-v2-youtube-supabase-vertical-slice.md` · **Next:** optional Contents→Supabase YouTube cutover planning（別承認）· または dual-path 維持。

**CMS Core v2 YouTube Supabase owner remote dry-run PASS + Save round-trip preflight (2026-07-24):** superseded by Save round-trip COMPLETE above.
