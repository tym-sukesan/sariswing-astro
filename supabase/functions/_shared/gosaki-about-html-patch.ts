// @ts-nocheck — mirrored from Node patch lib; types declared above for consumers.
/**
 * Gosaki About content — structured field ↔ HTML surgical patch (Edge).
 * Keep in sync with tools/static-to-astro/scripts/lib/gosaki-about-content-html-patch.mjs
 */
...
export type AboutProfileSnapshot = {
  heading: string;
  body: string;
  imageAlt: string;
};

export type AboutBandSnapshot = {
  id: string;
  name: string;
  body: string;
  imageAlt: string;
...
export type AboutContentSnapshot = {
  profile: AboutProfileSnapshot;
  bands: AboutBandSnapshot[];
};
