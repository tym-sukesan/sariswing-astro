...
assert("google-forms renders iframe mode", gfResult.ok && gfHtml.rendered === "google-forms");
assert("google-forms has iframe marker", /data-external-form-iframe=["']1["']/.test(gfHtml.html));
assert("google-forms no external-link marker", !/data-external-form-link=/.test(gfHtml.html));
assert("google-forms no script", !/<script\b/i.test(gfHtml.html));
