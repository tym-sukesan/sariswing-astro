...
**CMS Core v2 About Supabase Admin read/hydrate local implementation (2026-07-28):** **COMPLETE (local only · no Edge deploy)** — Edge `operation=read` SELECT-only · JWT+`can_write_site` · allowlist about/`profile.lede` · Admin overlay lede only · JSON fallback · Save arms false · public About JSON unchanged · root↔mirror byte match · baseline `3e7daae` · Doc: `cms-core-v2-about-supabase-admin-read-hydrate-local-implementation.md` · Gates: `ABOUT_SUPABASE_ADMIN_READ_HYDRATE_LOCAL_IMPLEMENTATION_COMPLETE: true` · `EDGE_DEPLOY_EXECUTED: false` · `readyForAboutSupabaseAdminReadHydrateEdgeDeployPreflight: true` · **Next:** Edge deploy preflight (docs) → operator deploy → package/FTP QA.
