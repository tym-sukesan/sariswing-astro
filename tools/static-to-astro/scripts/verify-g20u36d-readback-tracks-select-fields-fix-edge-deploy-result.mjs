/**
 * G-20u36d-readback-tracks-select-fields-fix-edge-deploy-result verifier.
 * Result-record only — no re-deploy / SQL / Save / root edit.
 * Run: node tools/static-to-astro/scripts/verify-g20u36d-readback-tracks-select-fields-fix-edge-deploy-result.mjs
 */
...
if (headShort.stdout.trim() === BASE_COMMIT) {
  ...
  console.log(
    `NOTE HEAD is ${headShort.stdout.trim()} (G-20u36d tracks-select-fields edge-deploy-result base ${BASE_COMMIT}) — non-blocking`,
  );
}
...
assert(
  "doc phase G-20u36d-readback-tracks-select-fields-fix-edge-deploy-result-record",
  doc.includes("G-20u36d-readback-tracks-select-fields-fix-edge-deploy-result-record"),
);
assert(
  "doc gate edge deployed",
  doc.includes("gosakiDiscographyEdgeDryRunReadBackTracksSelectFieldsFixEdgeDeployed: true"),
);
...
assert(
  "doc TRACK_SELECT_FIELDS without duration",
  doc.includes("TRACK_SELECT_FIELDS") && doc.includes("duration") && /exclude|without|excludes/i.test(doc),
);
assert(
...
assert(
  "doc live verify retry-2 not executed",
  doc.includes("retry-2") ||
    doc.includes("Live verify retry-2") ||
    doc.includes("liveVerifyRetry2Executed: false"),
);
...
assert("doc next live-verify-retry-2", doc.includes("G-20u36d-readback-live-verify-retry-2"));

assert(
  "package script verify:g20u36d-readback-tracks-select-fields-fix-edge-deploy-result",
  packageJson.includes("verify:g20u36d-readback-tracks-select-fields-fix-edge-deploy-result"),
);
...
assert(
  ...
  currentState.includes("G-20u36d-readback-tracks-select-fields-fix-edge-deploy-result") ||
    currentState.includes("tracks-select-fields-fix-edge-deploy-result"),
);
assert(
  "AI next-actions live-verify-retry-2",
  nextActions.includes("G-20u36d-readback-live-verify-retry-2") ||
    nextActions.includes("live-verify-retry-2"),
);
assert(
  "AI handoff tracks edge deploy result",
  handoff.includes("tracks-select") &&
...
console.log(
  `\nG-20u36d-readback-tracks-select-fields-fix-edge-deploy-result: ${passed} passed, ${failed} failed`,
);
