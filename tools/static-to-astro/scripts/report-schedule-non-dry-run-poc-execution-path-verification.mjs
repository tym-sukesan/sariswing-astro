#!/usr/bin/env node
/**
 * G-6-e5-schedule-non-dry-run-poc-execution-path-verification — Report CLI (static scan).
 */
...
import {
  DEFAULT_TOOL_ROOT,
  runScheduleNonDryRunPocExecutionPathVerificationReport,
  writeScheduleNonDryRunPocExecutionPathVerificationOutput,
} from "./lib/schedule-non-dry-run-poc-execution-path-verification-reporter.mjs";
...
function main() {
  ...
  if (opts.help) {
    console.log(
      "Usage: node scripts/report-schedule-non-dry-run-poc-execution-path-verification.mjs [--out-dir PATH]",
    );
    process.exit(0);
  }
  ...
  const report = runScheduleNonDryRunPocExecutionPathVerificationReport({
    toolRoot: DEFAULT_TOOL_ROOT,
    siteId,
  });

  console.log("static-to-astro report-schedule-non-dry-run-poc-execution-path-verification");
  console.log(`  phase: ${report.phase}`);
  console.log(`  executionPathImplemented: ${report.executionPathImplemented}`);
  console.log(`  executionPathVerified: ${report.executionPathVerified}`);
  ...
  console.log(
    `  readyForManualEnvGatedBrowserVerification: ${report.readyForManualEnvGatedBrowserVerification}`,
  );
  console.log(
  ...
  if (opts.outDir) {
    ...
    const { jsonPath, mdPath } =
      writeScheduleNonDryRunPocExecutionPathVerificationOutput(absOut, report);
  ...
  console.log("OK — schedule non-dry-run PoC execution path verification report complete.");
}
