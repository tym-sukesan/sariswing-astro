...
| Admin 運用方法の確定 | ローカル Admin / 別ホスト Admin / export 手順のいずれかを決定 |
...
Phase G-4: 画像 Storage 方針決定（+ staging upload 実装）
...
- [admin-hosting-strategy.md](./admin-hosting-strategy.md) のパターンから 1 つ選択
- export → build → public-dist の Runbook
- Sariswing の「公開更新」UX を参考に、gosaki 向け最小 UX を定義
...
| [site-profile-system.md](./site-profile-system.md) | profile 設計 |
