...
**CMS Core v2 About Supabase vertical slice preflight (2026-07-24):** **COMPLETE (docs + SQL templates only)** — schema **`site_page_fields`** (keyed scalars; opaque HTML rejected) · first field `about`/`profile.lede` · reuse `sites`/`site_members`/`platform_admins`/`can_write_site` · fail-closed RLS + column GRANT · Contents fallback · new approvals `G-cms-v2-about-supabase-profile-lede-*` (not G-12a / YouTube) · access = reuse YouTube membership (no new INSERT) · templates migration/RLS/seed + rollbacks · Doc: `cms-core-v2-about-supabase-vertical-slice-preflight.md` · Verifier: `verify-cms-core-v2-about-supabase-vertical-slice-preflight.mjs` · Gates: `cmsCoreV2AboutSupabaseVerticalSlicePreflightComplete: true` · `readyForOperatorAboutMigrationApply: false` · `aboutSupabaseImplementationExecuted: false` · **no** SQL apply/DB/Edge/FTP/production · **Next:** operator accept preflight → separate apply-gate phase → then local Edge/admin implementation · Save arm false 維持。
