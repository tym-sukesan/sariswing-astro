# G-20u36d — Gosaki Discography Edge dry-run readBack tracks SELECT fields fix edge deploy result
...
**Date:** 2026-07-13  
**Base commit:** `3bc0d4c`  
**Prior:** G-20u36d readBack tracks SELECT fields fix edge deploy preflight · root placement · env secret setting result
...
| Live verify retry-2 | **not executed** |
| SQL executed | **no** |
| DB write | **no** |
| Save enablement | **no** |
| Admin UI change | **no** |
...
```txt
gosakiDiscographyEdgeDryRunReadBackTracksSelectFieldsFixEdgeDeployed: true
phase: G-20u36d-readback-tracks-select-fields-fix-edge-deploy-result-record
...
liveVerifyRetry2Executed: false
...
proceedToLiveVerifyRetry2: true
proceedToG20u36eSavePlanning: false
```

**G-20u36d readBack tracks-select-fields-fix edge-deploy-result-record scope:** record operator deploy outcome only. No re-deploy, no SQL, no Save enablement, no admin UI change, no root function edit.
...
| Item | Status |
| --- | --- |
| Deploy preflight | **complete** — `G-20u36d-readback-tracks-select-fields-fix-edge-deploy-preflight` |
| Root tracks SELECT fields fix | **placed** — `TRACK_SELECT_FIELDS` without **`duration`** · `track_number/title/sort_order/site_slug` only |
| Root release-id fix | **retained** — `RELEASE_SELECT_FIELDS` + internal `id` · tracks `release_id` lookup |
...
**Deployed Edge Function code is now tracks-select-fields fix + release-id fix + readBack-capable** (`TRACK_SELECT_FIELDS` excludes **`duration`** · staging columns only · `RELEASE_SELECT_FIELDS` includes internal `id` · sanitized readBack summary · anon SELECT only). Live readBack `trackCount=8` behavior is **not yet verified** — see next phase.
...
| Item | Status |
| --- | --- |
| Staging Edge endpoint | tracks SELECT fields fix + release-id fix + readBack code **deployed** |
| Prior live verify retry (PARTIAL STOP) | `trackCount=0` · matching dryRun **400** — `duration` column missing on deployed Edge |
| Expected after live verify retry-2 | `readBack.trackCount=8` · matching dryRun **200** · `wouldWrite=false` |
