...
export function getG15bDiscographyPurchaseUrlSaveConfig(
  ...
  const envArmArmed = isEnvArmTrue(mergedEnv, G15B_DISCOGRAPHY_PURCHASE_URL_NON_DRY_RUN_ARMED_ENV);
  const dryRun =
  ...
  const base = {
    ...
    envArm: G15B_DISCOGRAPHY_PURCHASE_URL_NON_DRY_RUN_ARMED_ENV,
    defaultDisabledReason: G15B_DISCOGRAPHY_SAVE_DISABLED_DEFAULT_REASON,
    dev,
    stagingShellEnabled,
    ...
    supabaseConfigured,
    productionBlocked,
    expectedProject: G15B_DISCOGRAPHY_EXPECTED_PROJECT,
    expectedSupabaseHost: G15B_DISCOGRAPHY_EXPECTED_SUPABASE_HOST,
  ...
  if (approvalIdEnv !== G15B_DISCOGRAPHY_PURCHASE_URL_NON_DRY_RUN_APPROVAL_ID) {
    armFailures.push(
      `PUBLIC_ADMIN_WRITE_APPROVAL_ID=${G15B_DISCOGRAPHY_PURCHASE_URL_NON_DRY_RUN_APPROVAL_ID}`,
    );
  }
  if (dryRun) armFailures.push("PUBLIC_ADMIN_WRITE_DRY_RUN=false");
  if (!envArmArmed) {
    armFailures.push(`${G15B_DISCOGRAPHY_PURCHASE_URL_NON_DRY_RUN_ARMED_ENV}=true`);
  }
  ...
  const compileSaveEnabled =
    String(mergedEnv[G15B_DISCOGRAPHY_SAVE_ENABLED_ENV] ?? "").trim() === "true";
