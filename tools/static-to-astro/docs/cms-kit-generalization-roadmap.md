...
**G-5y-d（完了）:** [staging-supabase-auth-connection.md](./staging-supabase-auth-connection.md) — staging shell Supabase Auth（env gate、`getSession` / `signInWithPassword` / `signOut`）。mock fallback。次: **G-5y-e** staging role check / allowlist。
